{
платежи по 12 месяцам за выбранный год, сгруппированные или по статьям раходов, или по их группам.
также выводится диаграмма и общая задолженность.
}
...
begin
  ...
  Frg1.CreateAddControls('1', cntNEdit, 'Отчечка', 'NeSplice', '', 220 + 40,  yrefC, 60);
  Frg1.CreateAddControls('1', cntNEdit, 'Начальная задолженность', 'NeInitialDebt', '', 220 + 50 + 200, yrefC, 60);
...
begin
  ...
  'union '+
  '(select to_char(c.dt, ''mm'') month, sum(c.sum) sum1, sum(c.sum) sum, ''Прочие расходы'' as ie_name '+
  '  from'+
  '    sn_cash_payments c'+
  '  where'+
