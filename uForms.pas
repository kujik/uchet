...
//
//ВАЖНО: контролы с Visible = False на момент вызова полностью ИГНОРИРУЮТСЯ (см. "if not c.Visible then Continue"
//в реализации) - под них не резервируется место, они не входят ни в разбиение на строки, ни в расчет Result
//(итоговых ширины/высоты контейнера). Это осознанное поведение (например, чтобы диалог не резервировал место
//под условно скрытые поля), но у него есть следствие: если в TFrmBasicMdi.CorrectFormSize/FormShow этот вызов
//происходит ДО того, как форма определила, что какие-то контролы должны стать видимыми (типичный случай -
//видимость контрола зависит от режима/выбора пользователя и устанавливается уже ПОСЛЕ Prepare), то при
//последующем появлении такого контрола место под него уже не будет зарезервировано - контрол окажется ниже
//реально посчитанной высоты формы/панели и будет визуально не виден (хотя Visible = True), пока пользователь
//не растянет форму вручную (см. также FWHBounds в uFrmBasicMdi.pas). Готовый паттерн для этого случая - см.
//комментарий у TFrmBasicMdi.AfterFormActivate: держать такие контролы видимыми на момент первого показа формы
//(т.е. на момент вызова из CorrectFormSize), чтобы под них зарезервировалось место, а их реальную (скрытую)
//видимость применять уже в AfterFormActivate, когда это место уже посчитано. Конкретный пример реализации -
//TFrmODedtOrStdItem (uFrmODedtOrStdItem.pas): SetTabsControlsState/AfterFormActivate.
...
//      c:= TPanel(AParent).Controls[Ctrls[Integer(CtrlsH[i][j])][0]];
      ...
      if c is TBevel then begin
        //ВАЖНО: раньше здесь было "h := 3", т.е. общая для всей строки переменная h (высота строки, используемая
        //ниже и для центрирования ОСТАЛЬНЫХ контролов этой же строки, и для продвижения t на следующую строку)
        //перезаписывалась высотой бевела. Пока бевел - единственный контрол в своей строке (обычный случай -
        //бевел-разделитель на всю ширину), это было незаметно (CtrlsHH[i] и так уже маленькая). Но если бы бевел
        //оказался в одной строке с более высокими соседями, это ломало бы их центрирование и продвижение t для
        //всех последующих строк. Высоту в 3px оставляем только для самого бевела (визуально), не трогая h.
        c.Top:= t + (h - 2) div 2;
        c.Height:= 3;
      end
      else if c is TLabel then begin
...
procedure TControlsHelper.FillComboBoxWithBiweeklyPeriods(ComboBox: TDBComboBoxEh; ADate: TDateTime; APeriodCount: Integer);
var
  Periods: TStringList;
...
begin
  ...
  try
    // Определяем, в каком периоде находится ADate
    ...
    for i := 0 to APeriodCount - 1 do begin
      ...
      if DayOf(CurrentStart) = 1 then begin
        // Текущий период — первая половина → предыдущий: вторая половина прошлого месяца
        ...
        // Текущий период — вторая половина → предыдущий: первая половина того же месяца
    ...
    // Теперь переместим период, содержащий ADate, на самый верх
    TargetPeriodIndex := -1;
    for i := 0 to Periods.Count - 1 do begin
      // Распарсим даты из строки
      CurrentStart := StrToDate(Copy(Periods[i], 1, 10));
      CurrentEnd := StrToDate(Copy(Periods[i], 14, 10));
      if (ADate >= CurrentStart) and (ADate <= CurrentEnd) then begin
        TargetPeriodIndex := i;
        Break;
      end;
    end;
    ...
    try
      ...
      if TargetPeriodIndex >= 0 then begin
