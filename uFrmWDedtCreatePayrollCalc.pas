unit uFrmWDedtCreatePayrollCalc;
...
type
  TFrmWDedtCreatePayrollCalc = class(TFrmBasicMdi)
...
var
  FrmWDedtCreatePayrollCalc: TFrmWDedtCreatePayrollCalc;
...
procedure TFrmWDedtCreatePayrollCalc.cmbPeriodWChange(Sender: TObject);
begin
  if cmbPeriodD.Text <> '' then
    FillWorkers;
end;

procedure TFrmWDedtCreatePayrollCalc.FillWorkers;
var
  dt1, dt2: TDateTime;
begin
...
//  Q.QLoadToDBComboBoxEh('select distinct employee_st from v_w_employee_properties where is_terminated = 1 and dt_beg >= :dt1$d and dt_beg <= :dt2$d order by employee_st', [dt1, Turv.GetTurvEndDate(IncDay(dt2, 1))], cmbWorker, cntComboL);
  Q.QLoadToDBComboBoxEh('select distinct employee_st from v_w_employee_properties where is_terminated = 1 and dt_beg >= :dt1$d and dt_beg <= :dt2$d order by employee_st', [dt1, IncDay(IncMonth(dt1, 2), -1)], cmbWorker, cntComboL);
  cmbWorker.ItemIndex := 0;
end;

function TFrmWDedtCreatePayrollCalc.VerifyAdd(Sender: TObject; onInput: Boolean = False): Boolean;
begin
  Result := (pgcMain.ActivePageIndex = 0) and (cmbPeriodD.Text = '') or (pgcMain.ActivePageIndex = 1) and ((cmbPeriodW.Text = '') or (cmbWorker.Text = ''));
end;

procedure TFrmWDedtCreatePayrollCalc.btnOkclick(Sender: TObject);
var
  dt1, dt2: TDateTime;
  i, j: Integer;
  ...
  if pgcMain.ActivePageIndex = 0 then begin
    ...
    for i := 0 to High(va1) do begin
      if A.PosInArray(va1[i][0], va2, 0) = -1 then begin
        ...
        if Q.QSave('i', 'w_payroll_calc', '', 'id$i;id_departament$i;dt$d', [0, Integer(va1[i, 0]), dt1], False) <> -1 then
...
//      [cmbWorker.Text, dt1, Turv.GetTurvEndDate(IncDay(dt2, 1))], va1
    ...
    if b then begin
      ...
      Q.QLoad(
        'select id, id_target_departament, is_finalized from v_w_payroll_calc_item where id_target_employee is null and dt = :dt1$d and id_employee = :id_employee$i ' +
      ...
      if b then
        for i := 0 to High(va1) do begin
          ...
          if A.PosRowInArray(va1, va2, i) = -1 then begin
            //создаем
            if Q.QSave('i', 'w_payroll_calc', '', 'id$i;id_departament$i;id_employee$i;id_organization$i;personnel_number$s;dt$d', [0, va1[i][0], va1[i][1], va1[i][2], va1[i][3], dt1], False) <> -1 then
              inc(Cnt);  //увеличим количество созданных
          end;
        end;
    end;
...
function TFrmWDedtCreatePayrollCalc.Prepare: Boolean;
begin
  Result := False;
  ...
  Cth.FillComboBoxWithMonthlyPeriods(cmbPeriodD, Date, S.IIf(User.IsDeveloper, 12, 2));
  cmbPeriodD.ItemIndex := 0;
  Cth.FillComboBoxWithMonthlyPeriods(cmbPeriodW, Date, S.IIf(User.IsDeveloper, 12, 2));
