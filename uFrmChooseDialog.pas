{
  ћодуль uFrmChooseDialog предоставл€ет диалоговое окно дл€ выбора пользователем одного варианта из списка.

  Ќазначение:
    ќтображает форму с заголовком, текстовым сообщением (опционально), группой радиокнопок (TRadioGroup)
    и стандартными кнопками OK/Cancel (унаследованными от TFrmBasicMdi). ѕользователь выбирает один из предложенных
    вариантов, после чего диалог возвращает индекс выбранного элемента.

  ќсобенности:
    - јвтоматический расчЄт ширины формы: если параметр AWidth = 0, то ширина определ€етс€ по самому длинному тексту
      варианта (с использованием вспомогательного метода Cth.GetTextWidth, предположительно из модул€ uString).
    - јвтоматический расчЄт высоты текстовой метки (lblText) с переносом слов, чтобы текст целиком поместилс€
      в заданную ширину (используетс€ Cth.GetWordWrapHeight).
    - ¬ысота формы вычисл€етс€ динамически на основе количества вариантов, высоты панели с текстом и панели кнопок.
    - ѕоддерживаетс€ предустановленный вариант по умолчанию (ADefaultVariant).
    - –езультат работы метода ShowDialog: индекс выбранного элемента или -1 при отмене.
}
...
type
  ...
  public
      ...
      //AText           - текст по€снени€ (отображаетс€ выше радиогруппы). ≈сли пусто, панель с текстом скрываетс€.
      //AVariants       - массив вариантов (TVarDynArray) дл€ отображени€ в радиогруппе.
      //AInfo           - дополнительна€ информаци€ (тип TVarDynArray2), передаЄтс€ в базовый PrepareCreatedForm.
      //AWidth          - принудительна€ ширина формы в пиксел€х (0 = авто-расчЄт по самому длинному варианту).
      //ADefaultVariant - индекс предварительно выбранного варианта (-1 = ничего не выбрано).
      //¬озвращает: индекс выбранного варианта (0..High(AVariants)) или -1, если пользователь нажал Cancel.
