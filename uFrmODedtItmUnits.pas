...
//  CtrlValuesDefault:=[-1, 0, '', '', 1];
  FOpt.InfoArray:=[[
    'Ввод единицы измерения.'#13#10+
    'Если будет найдена похожая единица измерения, то будет выдыно предупреждение!'#13#10+
    ''#13#10
  ]];
...
//проверим, нет ли похожей единицы измерения (по краткому наименованию)
//для этого приведем к верхнему регистру и уберем из наименований точки и пробелы
//в итм нет даже уникальногшо индекса по имени ед.изм.
//если вернет в Result True, то будет выдано сообщение "Данные не корректны!"
var
  va: TVarDynArray;
  i: Integer;
  st1, st2: string;
begin
  ...
  for i := 0 to High(va) do
    if st1 = S.ToUpper(StringReplace(StringReplace(va[i], ' ', '', [rfReplaceAll]), '.', '', [rfReplaceAll])) then
      Break;
  if i <= High(va) then
    FErrorMessage := '?Единица измерения с похожим названием уже существует:'#13#10 + S.NSt(va[i]) + #13#10'Все равно создать?'
  else
    FErrorMessage := '';
end;
