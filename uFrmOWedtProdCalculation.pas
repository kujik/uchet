...
begin
  ...
  for var i := 1 to 5 do begin
    ...
    if  i = 1 then begin
      Frg1.Opt.SetPick('facing_id_banding_type', ['Авто', 'С 1й стороны', 'С 2х сторон'], [-1, 1, 2], True, True);
...
begin
  ...
  if No = 1 then begin
   Fr.SetValue('consumption', Row,False, Fr.GetValueF('length', Row, False) * Fr.GetValueF('width', Row, False) * Fr.GetValueF('waste', Row, False) * Fr.GetValueF('qnt', Row, False));
    ...
    if Fr.GetValueI('facing_id_banding_type', Row, False) = -1 then
    ...
    1:
      begin
        ...
        AFields:=[
          ...
          ['id_facing_name$i','_id_name','40','t=1'],
          ['changed$i','_ch','40'],
          ['error$i', 'x', '25', 'pic=0;1:1;6'],
          ['assembly$s','Материалы основвы|Узел изделия','150','t=1','e=0:100::T'],
          ['panel_type$s','!Панель|Тип','150','t=1','e','e=0:100::T'],
          ...
          ['price0$f','!Закупка, руб/кв.м.','60','f=r','t=1'],
          ...
          ['facing_price0$f','!Закупка, руб/м.кв.','60','f=r','t=1'],
    ...
    2:
      begin
        ...
        AFields:=[
          ...
          ['price0$f','Закупка, руб/м.п.','60','t=1','f=r'],
          ['sum0$f','Закупка, сумма','60','f=r:'],
          ['markup$f','Наценка','60','t=1','e=0:100:2'],
          ['sum$f','Продажа, сумма','60','f=r:']
        ];
    ...
    4:
      begin
         ...
         AFields:=[
          ...
          ['price0$f','Закупка, руб/шт.','60','t=1','f=r'],
          ['sum0$f','Закупка, сумма','60','f=r:'],
          ['markup$f','Наценка','60','t=1','e=0:100:2'],
          ['sum$f','Продажа, сумма','60','f=r:']
        ];
...
begin
  ...
  for var i := 1 to 5 do begin
    ...
    LSalesSum := LPurchaseSum + Frg.GetFuterValue('sum', False).AsFloat;
    if i = 1 then begin
      LPurchaseSum := LPurchaseSum + Frg.GetFuterValue('facing_sum0', False).AsFloat;
      LSalesSum := LPurchaseSum + Frg.GetFuterValue('facing_sum', False).AsFloat;
    end;
  end;
