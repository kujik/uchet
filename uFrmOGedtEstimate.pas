...
begin
  ...
  //заголовочный лейбл
  FTitleTexts := [S.IIf(AddParam = 1, 'Смета к стандартному изделию ' +FFormatCaption + ':', 'Смета кизделию заказа:'),  {'$FF0000' + } FName];
  pnlTop.Height := 50;
  ...
  Frg1.Opt.SetFields([
    ...
    ['type_of_item$s','Изделие','85', 'e=1:1000','bt=Изделие:И:::009;Смета:С:::909', 'pic=;П;ПФ;Н;О:0;7;7;8;9:+','t=1'],
    ['id_group$i','Группа','250;w;L','e=1:100000::TP','t=1,2'],
    ['name$s','Наименование','400;w;h','e=1:1000',
      'bt=Выбрать материал:М:::090' + S.IIFStr(FTypeOfItem <> 2, ';Выбрать полуфабрикат:П:::909') + S.IIFStr(FTypeOfItem = 1, ';Выбрать производственное изделие:И:::009') + ';Выбрать нестандартное изделие:Н:::000','t=1'],
...
begin
  if (Fr.CurrField = 'err') and (Fr.GetValueS('err') <> '') then
    MyInfoMessage(Copy(Fr.GetValueS('err'), 3), 1);
end;
...
var
  Res : TVarDynArray;
begin
  res := Q.QCallStoredProc('p_test_estimate_item', 'g$ii;n$si;sg$ii;result$so;id_std_item$io;id_estimate$io', [Frg1.GetValue('id_group', Row, False), Frg1.GetValueS('name', Row, False), FGroupOfItem, '', -1, -1]);
  Frg1.SetValue('flags', Row, False, Res[3]);
  Frg1.SetValue('id_or_std_item', Row, False, Res[4]);
  Frg1.SetValue('id_item_estimate', Row, False, Res[5]);
  Frg1.SetValue('err', Row, False, Copy(Res[3], 1, 1));
