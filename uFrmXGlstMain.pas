  ...
  //журнал стандартных форматов паспортов
    if (FormDoc = myfrm_R_StdPspFormats) then begin
      ...
      if TFrmBasicInput.ShowDialog(FrmMain, '', [], fMode, 'Параметры сметы', 300, 80, [
        ...
        [cntEdit,'Префикс','1:20:0', 100],
        [cntCheck,'Используется','', 100]
       ], va, va, [['']], nil) < 0
      then Exit;
      if Q.QIUD(Q.QFModeToIUD(fMode), 'or_format_estimates', '', 'id$i;id_format$i;name$s;prefix$s;active$i',
        [Fr.ID, Frg1.ID, va[0], va[1], va[2]]) < 0 then
