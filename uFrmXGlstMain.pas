...
//      ['agreed2$i','Директор','80','chb=+','e',User.Roles([], [rPC_A_AgrDir])]
  ...
  //платежный календарь - группы статей расходов
   ...
   //платежный календарь - статьи расходов
    ...
    Frg1.Opt.SetFields([
      ...
      ['itm_needed$i','Счет в ИТМ','60','pic'],
  ...
  //платежный календарь - счета ИТМ - выбор из списка
  else if FormDoc = myfrm_R_Itm_Schet_SELCH then begin
    Caption:='Выбор счета ИТМ';
    Frg1.Opt.SetFields([
      ['id_schet$i','_id'],
      ['date_registr$d','Дата регистрации','70'],
      ['num$s','№ счета','120'],
      ['control_date$d','Дата счета','70'],
      ['name_org$s','Поставщик','200'],
      ['sum$f','Сумма','70','f=r']
    ]);
    Frg1.Opt.SetTable('v_itm_accounts');
    Frg1.Opt.SetButtons(1,[[mbtSelectFromList],[],[mbtRefresh],[],[mbtGridSettings],[mbtCtlPanel]]);
    Frg1.InfoArray:=[[Caption + '.'#13#10]
    ];
  end
  ...
  {РАБОТНИКИ}
    ...
    Frg1.Options := Frg1.Options + [myogGridLabels] - [myogSorting];
    Frg1.Opt.SetFields([
      ['id$i','_id','40'],
...
//      ['managername$s','!Менеджер','100'],
      ['status$s','Статус','120','pic=принят;уволен;переведен:7;6;1:+'],
      ['dt_beg$d','С','80'],
      ['dt_end$d','По','80'],
      ['prev_departament$s','Подразделение|до','200;h'],
      ['departament$s','Подразделение|после','200;h'],
      ['prev_job$s','Должность|до','200;h'],
      ['job$s','Должность|после','200;h'],
//      ['grade$f','!Разряд','60'],
      ['prev_schedule$s','График|до','70'],
      ['schedule$s','График|после','70'],
      ['prev_organization$s','Организация|до','100'],
      ['organization$s','Организация|после','100'],
      ['personnel_number$s','Табельный номер','90'],
      ['comm$s','Комментарий','200;h']
    ]);
    Frg1.Opt.SetTable('v_w_employee_status');
    Frg1.Opt.SetWhere('order by name, dt_beg, id');
...
{  else if FormDoc = myfrm_J_Turv then begin
    ...
    Frg1.Opt.SetFields([
      ...
      ['dt1','Нач. дата','75'],
      ['dt2','Кон. дата','75'],
      ['committxt','Закрыт','60','pic=закрыт:13'],
...
//    Frg1.CreateAddControls('1', cntCheck, 'Прошлый период', 'ChbPrevious', '', 4, yrefB, 120);
  ...
  else if FormDoc = myfrm_J_AdvanceCalculations then begin
    Caption:='Авансовые расчетные ведомости';
...
//      ['dt2','Кон. дата','75'],
    ...
    Frg1.Opt.SetTable('v_w_advance_calc', 'w_advance_calc');
    Frg1.Opt.SetButtons(1, [
  ...
  else if FormDoc = myfrm_J_AdvanceTransfer then begin
    Caption:='Авансовые ведомости к перечислению';
    Frg1.Options := Frg1.Options + [myogGridLabels, myogIndicatorCheckBoxes, myogMultiSelect];
    Frg1.Opt.SetFields([
      ['id$i','_id','40'],
      ['employee','Работник','200'],
...
//      ['dt2','Кон. дата','75'],
    ...
    Frg1.Opt.SetTable('v_w_advance_transfer', 'w_advance_transfer');
    Frg1.Opt.SetButtons(1, [
      [mbtRefresh],[],[mbtView],[mbtEdit, User.Role(rW_J_Payroll_Ch)],[mbtAdd, 1],[],[mbtLock, User.Role(rW_J_Payroll_Ch), 'Закрыть выбранные'],
      [-1001, User.Role(rW_J_Payroll_Ch), 'Удалить выбранные', 'delete'],[],[mbtGridFilter],[],[mbtGridSettings],[],[mbtCtlPanel]
    ]);
  ...
  else if FormDoc = myfrm_J_AdvanceCash then begin
    Caption:='Авансовые ведомости к выдаче';
    Frg1.Options := Frg1.Options + [myogGridLabels, myogIndicatorCheckBoxes, myogMultiSelect];
    Frg1.Opt.SetFields([
      ['id$i','_id','40'],
      ['area','Площадка','150'],
    ...
    Frg1.Opt.SetTable('v_w_advance_cash', 'w_advance_cash');
    Frg1.Opt.SetButtons(1, [
...
//      'select distinct employee, id_employee from v_w_employee_properties where (dt_end is null or dt_end >= :dt1$d) and (dt_beg >= :dt2$d or is_terminated <> 1) and (:id$i = id_head) or 1 = :view_all$i)) order by employee',
 ...
 {   Frg1.Options := Frg1.Options + [myogGridLabels, myogLoadAfterVisible];
    ...
    Frg1.CreateAddControls('1', cntComboLK, 'Период:', 'CbPeriod', '', 60, yrefC, 150);
    Frg1.CreateAddControls('1', cntCheck, 'Включить ведомости по работникам:', 'chbAddW', '', 60 + 150 + 5, yrefC, 250);
    Q.QLoadToDBComboBoxEh('select to_char(dt1, ''dd-mm-yyyy'') || '' - '' || to_char(max(dt2), ''dd-mm-yyyy'') as dt from v_w_payroll_transfer group by dt1 order by dt1 desc', [],
      TDBComboBoxEh(Frg1.FindComponent('CbPeriod')), cntComboL
    );
    TDBComboBoxEh(Frg1.FindComponent('CbPeriod')).ItemIndex := 0;
  end
...
//Frg1.setinitdata('*',[]);
  ...
  //заказы - справочник причин рекламаций
  ...
  else if FormDoc = myfrm_Rep_Orders_Overdue_Kns_Thn then begin
    Caption:='Выполнение сроков конструкторами/технологами';
    Frg1.Options := Frg1.Options + [myogLoadAfterVisible];
    Frg1.Opt.SetFields([
      ['rownum$i','_id','40'],
      ['slash$s','№ заказа','100'],
      ...
      ['dt_beg$dt','Дата создания','80'],
      ['dt_otgr$dt','Плановая дата отгрузки','80'],
      ['dt_to_sgp_by_reglament$dt','Прием на СГП по регламену','80'],
      ['dt_to_sgp$dt','Прием на СГП факт.','80'],
      ['to_sgp_overdue_days$dt','Просрочка приема на СГП','80'],
      ['project$s','Проект','180;h'],
      ['customer$s','Покупатель','180;h'],
      ['type','Операция','150'],
      ['name','Работник','120'],
      ...
      ['overdue_days','Просрочка','70']
    ]);
    Frg1.Opt.SetTable('v_rep_orders_overdue_kns_thn');
    Frg1.Opt.SetButtons(1, 'rfs');
    Frg1.CreateAddControls('1', cntCheck, 'Только просроченные заказы', 'ChbOrOverdue', '', 4, yrefT, 200);
    Frg1.CreateAddControls('1', cntCheck, 'Только просрочка исполнителя', 'ChbOverdue', '', 4, yrefB, 200);
    ...
    Frg1.InfoArray:=[[
       ...
       '(прикрепеление документов и выдача документов в бумажном виде).'#13#10 +
       'В графе "Просрочка" отображается расхождение фактической даты выполнения операции с плановой (берется из регламента заказа).'#13#10 +
       'О нарушении сроков свидетельствует отрицательная просрочка.'#13#10+
       'Для выбора периода, за который отображаются данные, используйте кнопку фильтра в верхней части окна.'#13#10
    ]];
  end
  else if FormDoc = myfrm_Rep_Orders_Audit then begin
    Caption:='Журнал операций по заказам';
    Frg1.Options := Frg1.Options + [myogLoadAfterVisible];
    Frg1.Opt.SetFields([
      ['id$i','_id','40'],
    ...
    Frg1.Opt.SetTable('v_rep_orders_audit');
    Frg1.Opt.SetButtons(1, 'rfs');
    Frg1.Opt.FilterRules := [[], ['dt_date']];
  end
  ...
  else if FormDoc = myfrm_J_OrdersBySlashes then begin
    Caption:='Заказы по слешам';
    Frg1.Options := Frg1.Options + [myogLoadAfterVisible];
    Frg1.Opt.SetFields([
      ['id$i','_id','40'],
      ...
      ['slash','№','100'],
 //     ['area','Площадка','50'],
      ['dt_beg','Дата создания','100'],
      ['dt_otgr','Плановая дата отгрузки','100'],
      ['project','Проект','200'],
      ['customer','Заказчик','200'],
      ['or_reference$s','Основание','80'],
      ['ref_dt_beg$d','Основание: создан','75'],
      ['ref_dt_otgr$d','Основание: отгрузка (план)','75'],
...
//      ['is_semiproduct','Полуфабрикаты','80','pic'],
    ...
    Frg1.Opt.SetFields([
      ...
      ['price_check','Контр. цена','80','f=r']
    ]);
    Frg1.Opt.SetTable('v_itm_spschetspec');
  ...
  else if FormDoc = myfrm_R_Itm_DemandSupplier then begin
    //счет от поставщика из итм
    Caption:='Заявка поставщику';
    Frg1.Opt.SetFields([
      ['id_demand_supplier_spec$i','_id','40'],
      ['artikul','Артикул','120'],
      ['name','Наименование','300;h'],
      ['name_unit','Ед. изм','80'],
      ['quantity','Кол-во','70'],
...
//    st1 := '[№ документа $FF0000' + VarToStr(va1[0]) + '$000000 от $FF0000' + DateTimeToStr(va1[1]) + '$000000]    [$FF0000' +  VarToStr(va1[2]) + '$000000]    Поставщик: $FF0000' + VarToStr(va1[3]) + '$000000 ';
    st1 := '[№ документа $FF0000' + VarToStr(va1[0]) + '$000000 от $FF0000' + DateTimeToStr(va1[1]) + '$000000]   Поставщик: $FF0000' + VarToStr(va1[3]) + '$000000 ';
    Frg1.CreateAddControls('1', cntLabelClr, st1, 'LbCapt1', '', 5, yrefT, 800);
    Frg1.InfoArray := [['Заявка поставщику (из ИТМ).'#13#10'(Только просмотр.)']];
  end
...
{    if not(User.Roles([], [rOr_J_Tasks_Ch, rOr_J_Tasks_VAll])) then begin
    ...
    Frg1.Opt.SetFields([
      ...
      ['pos$i','№','40'],
      ['name$s','Наименование','200'],
      ['is_production_order$i','Производ-'#13#10'ственный','80','pic'],
      ['is_semiproduct_order$i','Полу-'#13#10'фабрикат','80','pic'],
      ['is_shipment_order$i','Отгру-'#13#10'зочный','80','pic'],
      ['is_complaint$i','Рекла-'#13#10'мация','80','pic'],
      ['is_additional_order$i','Доза-'#13#10'каз','80','pic'],
      ['is_reference_allowed$i','Допустима ссылка на заказ','80','pic'],
      ['is_reference_required$i','Обязательна ссылка на заказ','80','pic'],
    ...
    end;
{
    ...
    if (FormDoc = myfrm_J_Turv) and (fMode in [fDelete]) then
      if Turv.DeleteTURV(Fr.ID) then
        Fr.RefreshGrid;
    if (FormDoc = myfrm_J_AdvanceCalculations) and (fMode = fAdd) then
      TFrmWDedtCreateAdvance.Show(Self, '', [myfoDialog], fAdd, Fr.ID, null);
    if (FormDoc = myfrm_J_AdvanceCalculations) and (fMode in [fEdit, fView]) then
      Wh.ExecDialog(myfrm_Dlg_AdvanceCalc, Self, [], fMode, Fr.ID, null);
    if (FormDoc = myfrm_J_AdvanceTransfer) and (fMode = fAdd) then
      if Turv.CreateAllAdvanceTransfers then
        Fr.RefreshGrid;
    if (FormDoc = myfrm_J_AdvanceTransfer) and (fMode in [fEdit, fView]) then
      Wh.ExecDialog(myfrm_Dlg_AdvanceTransfer, Self, [], fMode, Fr.ID, null);
    if (FormDoc = myfrm_J_AdvanceCash) and (fMode = fAdd) then
      if Turv.CreateAllAdvanceCash then
        Fr.RefreshGrid;
...
//      TFrmWDedtCreatePayrollN.Show(Self, '', [myfoDialog], fAdd, Fr.ID, null);
    ...
    if ((FormDoc = myfrm_J_Devel_Ref) or (FormDoc = myfrm_J_DevelThn_Ref)) and ((fMode = fAdd) or (fMode = fEdit)) then begin
      if TFrmBasicInput.ShowDialog(Self, '', [], fMode, '~Вид работ', 500, 110,
         [[cntEdit, 'Наименование', '1:400:0:T'],[cntNEdit, 'Время, ч.', '0:1000000000:1:N'], ['active$i', cntCheckX, 'Используется']],
         [S.IIfStr(fMode = fEdit, Frg1.GetValueS('name')), S.IIf(fMode = fEdit, Frg1.GetValue('hours'), null), Frg1.GetValue('active')],
         va, [['caption dlgedit dlgactive']], nil
      ) < 0 then Exit;
      if fMode = fAdd then
        Q.QExecSql('insert into ref_develtypes (developer, name, hours, active) values (:developer$i, :name$s, :hours$f, :active$i)', [S.Decode([FormDoc, myfrm_J_Devel_Ref, 1, 2]), va[0], va[1], va[2]], False)
      else
        Q.QExecSql('update ref_develtypes set name = :name$s, hours = :hours$f, active = :active$i where id = :id$i', [va[0], va[1], va[2], Fr.ID], False);
      Fr.RefreshGrid;
    end;
...
{    if FormDoc = myfrm_R_OrderTypes then
  ...
  else if A.InArray(FormDoc, [myfrm_J_AdvanceCalculations, myfrm_J_AdvanceTransfer, myfrm_J_AdvanceCash, myfrm_J_PayrollCalculations, myfrm_J_PayrollTransfer, myfrm_J_PayrollCash]) and (Tag = 1001) then begin
...
{  else if (FormDoc = myfrm_J_PayrollTransfer) or (FormDoc = myfrm_J_PayrollCash) then begin
    ...
    SqlWhere := A.ImplodeNotEmpty([SqlWhere,
      S.IIfStr(Cth.GetControlValue(Fr, 'ChbOverdue') = 1, 'overdue_days < 0'),
      S.IIfStr(Cth.GetControlValue(Fr, 'ChbOrOverdue') = 1, 'to_sgp_overdue_days < 0')
      ], ' and '
    )
    ...
    Fr.SetSqlParameters('id$i', [AddParam])
...
{    FOpt.StatusBarMode := stbmDialog;
  ...
  //если изменяем данные во внутреннем массиве, для их отрисовке в гриде делаем:
  Frg1.DbGridEh1.Invalidate;

  //для загрузки данных из массива можно установить так:
  Frg1.SetInitData([], {'id;job;workersqnt;qnt1;qnt2;qntvac;dtvac'}, {'0;1;2...'});
  //если не задан список полей, загружаются все поля грида в порядке как в массиве, иначе сопоставляются по именам, а если заданы еще и колонки, то онни указывают соотвестви позиции поля имени колонке в массиве
  //или:
