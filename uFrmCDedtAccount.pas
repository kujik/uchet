//!! если очистить файл счета на червере, открыть счет, подгрузить - не сбарасывается статутс некорректные данные в таксбаре и не сохранить
//доделать для маршрута (галки маршрут, список рейсов)
...
begin
  ...
  if A.InArray(F.GetPropB('accounttype'), [1, 2]) then begin
    if Mode in [fEdit, fView, fDelete] then begin
      va2 := Q.QLoad(Q.QGetSql('s', 'sn_calendar_accounts_t', 'id$i;cartype$i;flighttype$i;flightdt;kilometrage;idle;basissum;othersum;priceidle;pricekm;freeidletime'), [ID]);
...
begin
  ...
  if User.Role(rPC_A_ChAll)
    then begin
      ...
      FExpenseItemsNeedReceipt := Q.QLoad('select recvreceipt, id from ref_expenseitems where (accounttype = :accmode$i and active = 1) or id = :id_expenseitem$i order by name',
      ...
      FExpenseItemsNeedReceipt := Q.QLoad('select recvreceipt, id from ref_expenseitems where (anyinstr(useravail, :id_user$i)=1 and accounttype = :accmode$i) or id = :id_expenseitem$i order by name',
  ...
  FExpenseItemsAgreed := Q.QLoad('select agreed, id from ref_expenseitems', []);
  ...
  if (Mode <> fView) and (Mode <> fDelete) then begin
      ...
      FPoints1 := Q.QLoadCol('select name from ref_sn_locations where type = 0 and active = 1 order by name', []);
      FPoints2 := Q.QLoadCol('select name from ref_sn_locations where type = :type order by name', [F.GetPropB('accounttype')]);
    end;
  end;
  Result := True;
end;
...
begin
  //по непонятной причине при изменении  первой ячейки на нулл (в том числе если первой операцией сделать выбор пункта назначения), происходит вставка какого-то числа в первую ячейку!
...
begin
  ...
  if Mode in [fAdd, fCopy] then
    Exit;
  Q.QLoad('select id, dt, sum, status from sn_calendar_payments where id_account = :id$i order by status desc, dt, sum', [ID], FPayments);
  FIsPaid := False;
...
begin
  ...
  if FrgRoute.GetCount(False) > 0 then begin
    ...
    if not A.InArray(st, FPoints1) then
      Q.QSave('i', 'ref_sn_locations', 'sq_ref_sn_locations', 'id;name;type;active', [0, st, 0, 1], False);
  end;
  ...
  for i := 0 to FrgRoute.GetCount(False) - 1 do begin
    ...
    if not A.InArray(st, FPoints2) then
      Q.QSave('i', 'ref_sn_locations', 'sq_ref_sn_locations', 'id;name;type;active', [0, st, FAccountType, 1], False);
  end;
  Result := True;
end;
...
var va :=GetControlValues([cmb_CarType, cmb_FlightType, dedt_FlightDt, nedt_Kilometrage], True);
  ...
  Q.QSave(Q.QFModeToIUD(Mode), 'sn_calendar_accounts_t', '-', 'id;cartype$i;flighttype$i;flightdt$d;kilometrage$f',   //;idle$f;basissum$f;othersum$f;pricekm$f;priceidle$f;freeidletime$f
...
begin
  ...
  if Cth.GetControlValue(cmb_id_ExpenseItem) = null then
    Exit;
  //может, если:
  //только когда не согласован директором, при праве rPC_A_AgrAll для люой или праве rPC_A_AgrSelfCat для своей (есть пользователь в поле Кто согласовывает для категории) категории
  LWhoAgreed := '';
