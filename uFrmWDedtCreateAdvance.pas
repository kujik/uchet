...
type
  TFrmWDedtCreateAdvance = class(TFrmBasicMdi)
...
var
  FrmWDedtCreateAdvance: TFrmWDedtCreateAdvance;
...
procedure TFrmWDedtCreateAdvance.cmbPeriodWChange(Sender: TObject);
begin
  if cmbPeriodD.Text <> '' then
    FillWorkers;
end;

procedure TFrmWDedtCreateAdvance.FillWorkers;
var
  dt1, dt2: TDateTime;
begin
...
function TFrmWDedtCreateAdvance.VerifyAdd(Sender: TObject; onInput: Boolean = False): Boolean;
begin
  Result := (pgcMain.ActivePageIndex = 0) and (cmbPeriodD.Text = '') or (pgcMain.ActivePageIndex = 1) and ((cmbPeriodW.Text = '') or (cmbWorker.Text = ''));
end;

procedure TFrmWDedtCreateAdvance.btnOkclick(Sender: TObject);
var
  dt1, dt2: TDateTime;
  i, j: Integer;
  LDeps, LAdvances, LDepsNoFinalized, va1, va2, va3: TVarDynArray2;
  va: TVarDynArray;
  Cnt: Integer;
  Msg: string;
...
//    S.GetDatesFromPeriodString(cmbPeriodD.Text, dt1, dt2);
    ...
    for i := 0 to High(LDeps) do begin
      ...
      if A.PosInArray(LDeps[i][0], LAdvances, 0) = -1 then begin
        j := A.PosInArray(LDeps[i][0], LDepsNoFinalized, 0);
...
function TFrmWDedtCreateAdvance.Prepare: Boolean;
begin
  Result := False;
  Caption := '~Добавить авансовые расчетные ведомости';
