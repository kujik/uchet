...
//  Q.QSave(Q.QFModeToIUD(Mode), 'bcad_nomencl', '', 'id$i;name$s;id_group$i;id_unit$i',
//    [ID, edt_name.Text, cmb_id_group.Value, cmb_id_unit.Value]
//  );
  Q.QSave(Q.QFModeToIUD(Mode), 'bcad_nomencl', '', 'id$i;name$s',
    [-1, edt_name.Text]
  );
  ...
  va1:= Q.QLoadCol('select artikul from dv.nomenclatura where id_group = :id_group$i', [FIdGroupItm]);
  va2:=[];
  ...
  if S.NSt(vai[0]) = '' then begin
    ID := Q.QSave(Q.QFModeToIUD(Mode), 'dv.nomenclatura', '', 'id_nomencl$i;name$s;fullname$s;id_group$i;id_unit$i;id_nomencltype$i',
