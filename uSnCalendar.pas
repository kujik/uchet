{
модуль общих функций дл€ платежного календар€
}
...
//перенос выбранных платежей на указанную дату
//перенесуны будут только те платежи, дол€ которых это возможно
//(не должны быть проведены, и счет должен быть доступен дл€ редактировани€ пользователем
//(или право редактировани€ по своим категори€м, или своих счетов)
//несколько платежей дл€ одного счета по одной дате проход€т и при редактировании счета и не провер€ютс€ также и здесь
var
  na: TNamedArr;
  va: TVarDynArray;
  i, cnt: Integer;
...
begin
  ...
  if Length(AIds) = 0 then begin
    MyInfoMessage('ќтметьте платежи, которые нужно перенести.');
    Exit;
  end;
  if Length(AIds) > 400 then begin
    MyInfoMessage('ƒл€ переноса можно выделить не более 400 платежей.');
    Exit;
  end;
  ...
  if cnt = 0 then begin
    MyInfoMessage('Ќельз€ изменить дату ни дл€ одного платежа.');
    Exit;
  end;
  if MyQuestionMessage(IntToStr(cnt) + ' из ' + S.GetEndingFull(na.Count, 'платеж', '', 'а', 'ей' ) + ' можно перенести.'#13#10'ѕродолжить?') <> mrYes then
    Exit;
  if TFrmBasicInput.ShowDialog(Application, '', [], fAdd, '~ѕеренести платежи', 150, 60, [[cntDEdit, 'ƒата', ':']], [null], va, [['']], nil) < 0 then
    Exit;
...
begin
  ...
  if TFrmBasicInput.ShowDialog(AOwner, '', [], FEdit, '~”даление старых счетов.', 220, 140,
    [[cntDTEdit,'”далить счета ранее: ', S.DateTimeToIntStr(EncodeDate(2026, 01, 01)) + ':' + S.DateTimeToIntStr(dtcash), 90, 0]],
    vin, vout, [['Ѕудут удалены счета ранее введенной даты по избранным стать€м, полностью оплаченные ранее этой же даты.']], nil
  ) < 0 then Exit;
  Q.QSetContextValue('delete_accounts_dt', vout[0]);
  cnt := Q.QLoadValue('select count(*) from v_get_outdated_accounts', []);
  if cnt = 0 then begin
    MyInfoMessage('Ќи один счет не будет удален.');
    Exit;
  end
  else if cnt > 1000 then
    MyWarningMessage('—лишком много счетов за данный период! ”меньшите период дл€ очистки!')
  else if MyQuestionMessage('Ѕудут безвозвратно удалены данные по ' + S.GetEndingFull(cnt, 'счет', 'у', 'ам', 'ам') + '.'#13#10'ѕродолжить?') <> mrYes then
    Exit;
  ShowWaitForm('”даление счетов...');
  ...
  if MyQuestionMessage('Ѕудут удалены перечисленные ниже счета. ѕродолжить?'#13#10#13#10 + dirs, 1) <> mrYes then
    Exit;
  ShowWaitForm('”даление счетов...');
//Exit;
  Q.QBeginTrans(True);
