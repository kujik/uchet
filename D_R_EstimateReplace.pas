...
begin
  ...
  if (Mode = fEdit)and(id <> id1) then begin
    //костыль
    //удаляем запись, если было изменение айди заменяемого наименования, тк ид в таблице отдельного у нас нет и общий запрос не пройдет из-за двух одинаковых параметров
  ...
  if Q.QSave(Q.QFModeToIUD(Mode),
    ...
    then begin MyWarningMessage('Ошибка сохранения данных!'); Exit; ModalResult:=mrOk; end;
  ModalResult:=mrOk;
end;
