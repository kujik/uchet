...
uses
  ...
  uString, uData, uMessages, uForms, uDBOra, uFrmBasicMdi, uFrDBGridEh, uFrmBasicGrid2, uFrmBasicInput
  ;
...
begin
  ...
  if (Fr.CurrField = 'dt_pln_ops') and User.Roles([], [rPln_R_PnlOpsPainting_V, rPln_R_PnlOpsPainting_Ch]) and (Fr.GetValueI('type') in [0, 2]) then begin
    Wh.ExecDialog(myfrm_Dlg_PnlOpsForItem, Self, [], S.IIf(User.Role(rPln_R_PnlOpsPainting_Ch), fEdit, fView), Fr.ID, THIS_IS_STD_ITEM);
  end;
end;
...
begin
  ...
  if A.InArray(FieldName, ['dt_influencing', 'dt_pln_ops']) then
