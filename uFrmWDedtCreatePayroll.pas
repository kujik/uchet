...
begin
  ...
  if pgcMain.ActivePageIndex = 0 then begin
    ...
    if not Cth.DteValueIsDate(dedtDt1) then
      Exit;
    if MyQuestionMessage('Создать зарплатные ведомости?') <> mrYes then
      Exit;
    ...
    va1 := Q.QLoadToVarDynArray2('select id_division, name, commit from v_turv_period where dt1 = :dt1$d', [dedtDt1.Value]);
    //получим список ведомостей по зп, по полным подразделениям за этот период
    va2 := Q.QLoadToVarDynArray2('select id_division from v_payroll where dt1 = :dt1$d and id_worker is null',  //and id_division = :id_division$i
      [dedtDt1.Value]);
...
{      if b then
  ...
  //      if QIUD('i', 'payroll', 'sq_payroll', 'id;id_division$i;dt1$d;dt2$d', [0, va1[i,0], dedt_Dt1.Value, dedt_Dt2.Value], False) <> -1
        ...
        if Q.QIUD('i', 'payroll', 'sq_payroll', 'id$i;id_division$i;dt1$d;dt2$d', [0, Integer(va1[i, 0]), dedtDt1.Value, dedtDt2.Value], False) <> -1 then
