...
иконка:
в uchet.dproj удалить строку Icon_MainIcon, перезапустить дельфи не обязательно
...
uses
  ...
  uFrmOGedtSupplierNomencl in 'uFrmOGedtSupplierNomencl.pas' {FrmOGedtSupplierNomencl},
  ...
  uFrmXAdmSqlCommentSync in 'uFrmXAdmSqlCommentSync.pas' {FrmXAdmSqlCommentSync},
  uSqlUpdaterCore in 'uSqlUpdaterCore.pas',
  uFrmXWViewFile in 'uFrmXWViewFile.pas' {FrmXWViewFile},
  uFrmXAdmSqlUpdater in 'uFrmXAdmSqlUpdater.pas' {FrmXAdmSqlUpdater},
  ...
  uFrmWGedtAdvance in 'uFrmWGedtAdvance.pas' {FrmWGedtAdvance},
  ...
  uFrmWGedtAdvanceCash in 'uFrmWGedtAdvanceCash.pas' {FrmWGedtAdvanceCash},
...
begin
  ...
  //(см. заголовок uServerTasks.pas), поэтому единственность процесса нужно
  //контролировать для модуля целиком: если учитывать параметр (как раньше),
  //процессы, запущенные с разными параметрами, могли бы работать резидентно
  //одновременно, дублируя выполнение задач и конкурируя за порт сканера
  //штрихкодов
  MT := CheckInstance(pChar(ModuleRecArr[cMainModule].FileNameEn + '-SrvOneOnly'));
  {$ELSE}
