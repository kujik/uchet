...
begin
  ...
  Q.QLoad(Q.QGetSql('A', 'v_prices_from_sp_schet', Fields.Col(0).Implode(';')) + ' where monitor_price = 1 and id_schet > :id$i order by name asc', [47900], na);
...
procedure TTasksS.ReportForYesterdayOrders(AOrderTypes: Boolean);
//финансовый отчет по производственным или отгрузочным заказам, запущенным за вчерашний день
//true - О, false - П
var
  na: TNamedArr;
...
begin
  ...
  TopSt := 'Отчет по ' + S.IIf(AOrderTypes, 'отгрузочным', 'производственным') + ' заказам за ' + DateTimeToStr(IncDay(Date, -1));
  Fields := [
    ['rownum$i', '№', '20'],
    ...
    ['price_std$i', 'Цена по справочнику', '80', 'r'],
    ['price$i', 'Цена продажи', '80', 'r'],
    ['price_diff$i', 'Занижение цены', '80', 'r'],
    ['summ$i', 'Сумма продажи', '80', 'f=#:', 's', 'r'],
    ['sum0$i', 'Себестоимость по смете', '80', 'f=#:', 's', 'r'],
    ['sum0_percent$i', '%', '80', 'r'],
  ...
  Q.QLoad(Q.QGetSql('A', 'v_orders_fin_monitoring', Fields.Col(0).Implode(';')) + ' where order_type = :p$i order by prime_cost_percent desc', [S.IIf(AOrderTypes, 0, -1)], na);
  if na.Count = 0 then begin
