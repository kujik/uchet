...
select
--НЕ ВЕРНО, не учитывает нд, не правильно в случе разницы кол-ва факт и по документу!
--последняя закупочная цена номенклатурной позиции по данным приходных накладных
--будет взята цена для максимального айди в ib_spec для данной номенклатуры
--никакие данные поставщика не учитываются, цена вычисляется в итм при создании накладной
  n.id_nomencl,
  ibs.ibprice as price_main
from  
  dv.nomenclatura n
  left outer join
  (select 
     max(id_ibspec) as id_ibspec,
     id_nomencl
   from
     dv.in_bill_spec
   group by
  ...
  left outer join
  dv.in_bill_spec ibs on ibs.id_ibspec = smax.id_ibspec
; 
...
from (
  select
    ...
    --ПЩ, кроме складов производства и "сгп и", "склад сырья и"    
    sum(decode(id_sklad, null, 0, 842, 0, 922, 0, 982, 0, 862, 0, 902, 0, quantity)) as qnt0,
...
where
  m.id_docstate = 3  --проведена
  and m.id_skladdest in (842, 922)  --на склад производства !!!
  and s.id_movebill(+) = m.id_movebill 
...
(select id_nomencl, sum(rezerv) as rezerv, sum(rezerv0) as rezerva0, sum(rezerv1) as rezerva1 from v_spl_rezerv_list group by id_nomencl) rz
