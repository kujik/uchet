...
select
  ...
  oi.id_kns as id_user,
  o.dt_beg + ri.day_end - 1 as dt_by_reglament,
  oi.dt_estimate as dt_fact,
  -(nvl(oi.dt_estimate, trunc(sysdate)) - (o.dt_beg + ri.day_end - 1)) as overdue_days 
from
  v_orders o,
  v_order_items oi,
  order_reglaments r,
...
where
  ...
  and oi.dt_estimate is null or at_est is not null  --уитываем строки, по которым сметы еще не подгружены или в любом случае если подгружены в ручном режиме
  and o.id_reglament is not null 
  and oi.id_order = o.id
  and o.id_reglament = r.id
  and ri.id_reglament = r.id and ri.id_work_cell_type = 1
