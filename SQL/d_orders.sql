...
create table order_items (
  ...
  price_final number,                --цена с НДС, скидкой и наценкой 
...
--обновим цены в шаблонах заказов
  ...
  v_nds_rate        number;      -- ставка ндс (%), берётся из контекста (устанавливается формой)
  v_price_wo_nds    number;      -- новая цена без ндс (пишется в price_base)
  v_price_with_nds  number;      -- новая цена с ндс (пишется в order_items.price для отгрузочных)
begin
...
--проверка пары связанных шаблонов заказа (см. orders.id_related_template): одинаковая ли у них
--группа форматов (id_format), образуют ли их подгруппы пару производственный/отгрузочный (см.
--or_format_estimates.type, STDITEM_TYPE_* в uOrders.pas), и находится ли табличная часть в
--синхронном состоянии (одинаковые по порядку позиции - совпадают наименования и количество).
--используется в диалоге связывания шаблонов (см. TOrders.LinkOrderTemplate в uOrders.pas) как
--информационная проверка. возвращает null, если все три проверки пройдены, иначе текст первой
--найденной проблемы
  AId1 in number,
  AId2 in number
) return varchar2
...
begin
  if (AId1 is null) or (AId2 is null) or (AId1 = AId2) then
    return 'Не выбрана пара шаблонов';
  end if;
  ...
  if FFormat1 <> FFormat2 then
    return 'Шаблоны относятся к разным группам форматов';
  end if;

  if not ((FType1 = 0 and FType2 = 1) or (FType1 = 1 and FType2 = 0)) then
    return 'Для связки нужна пара из производственного и отгрузочного шаблонов (STDITEM_TYPE_PRODUCTION/STDITEM_TYPE_SHIPMENT)';
  end if;

  select count(*) into FCnt1 from order_items where id_order = AId1;
  select count(*) into FCnt2 from order_items where id_order = AId2;
  if FCnt1 <> FCnt2 then
    return 'Разное количество позиций в табличной части шаблонов';
  end if;
  ...
  if FDiff > 0 then
    return 'Табличная часть шаблонов не синхронизирована (различаются наименования или количество позиций)';
  end if;
