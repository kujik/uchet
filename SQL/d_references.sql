...
create table ref_sn_organizations (
  ...
  is_buyer number(1),   --является покупателем (1) 
  is_seller number(1),   --является продавцом (1)
...
--адреса для транспортных счетов
...
create table ref_sn_locations (
  ...
  type number(1) default 0,   --0 - наше расположение, 1 - в счетах по транпорту ТО, 2- - счетах по транспорту СН
  active number(1), --активность
...
--типы транспортных средств для транспортных счетов
...
select
  t.*,
  decode(t.id_state, 1, 'новая', 2, 'в работе', 3, 'остановлена', 4, 'на согласовании', 5, 'зависла', 99, 'готово', '') as state, 
  u1.name as user1,
  u2.name as user2,
  oi.ornum,
  oi.slash,
