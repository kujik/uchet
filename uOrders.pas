...
type
  ...
  public
    ...
    function GetProductionOperations(AArea: Integer): TVarDynArray2;
...
const
  ...
  cOrderNew26TypeID = 18510; //100000000;
...
function TOrders.FinalizeOrdersM(AIds: TVarDynArray): Boolean;
//групповое завершение заказа менеджером
//заказы только завершаются, снятие отметки не предусмотрено
//выдается информация по заказам, завершить которые не удалось
var
  i: Integer;
  va2: TVarDynArray;
  IsRecl: Boolean;
...
begin
  ...
  if not User.Roles([], [rOr_D_Order_SetCompletedM, rOr_D_Order_SetCompletedMA]) then
    Exit;
  if Length(AIds) = 0 then begin
    MyInfoMessage('Отметьте заказы для завершения менеджером.');
    Exit;
  end
  else begin
    if MyQuestionMessage('Проставить отметку завершения менеджером для ' + S.GetEndingFull(Length(AIds), 'заказ', 'а', 'ов', 'ов') + '?') <> mrYes then
      Exit;
  end;
  ShowWaitForm('Ставим отметку завершения менеджером...');
  Q.QLoad(
    'select '+
    'o.id, o.prefix, o.ornum, o.dt_beg, o.dt_end, o.dt_to_sgp, o.dt_from_sgp, o.dt_upd, o.dt_end_manager, o.dt_montage_beg, o.id_type, o.id_manager, o.is_complaint, m.dt_end as dt_montage '+
    'from v_orders o, or_montage m '+
    'where m.id (+) = o.id and o.id in (' + AIds.Implode(',') + ')',
    [], na
  );
  msg := '';
  ids := [];
  for i := 0 to na.High do begin
    ...
    //можно менять у заказов, 'М','О','Ф' (одинаково), и 'Н' (если отгружен полностью)
    //П по рекламации менедежерам вообще нельзя закрыть
    if (na.G(i, 'dt_end')<> null) or not (A.InArray(na.G(i, 'prefix'), ['М', 'О', 'Ф']) or ((na.G(i, 'prefix') = 'Н') and (na.G(i, 'is_complaint') = 1))) then
      S.ConcatStrP(st, 'Не нужно закрывать', ' / ');
    //позволим менять статутс закрытия менеджера только сотрудникам, оформившим заказа,
    //или с соотвествующим правом, или с правами администратора данных
    if (st = '') and (User.GetId <> na.G(i, 'id_manager')) and not User.IsDataEditor and not User.Role(rOr_D_Order_SetCompletedMA) then
      S.ConcatStrP(st, 'Чужой заказ', ' / ');
    if (st = '') then begin
      //нет упд, не даем установить завершенным, но можно снять
      //но для рекламаций это не проверяем
      if (na.G(i, 'dt_upd') = null) and (na.G(i, 'is_complaint') <> 1) then
        S.ConcatStrP(st, 'Нет УПД', ' / ');
      //предупреждение, если не окончен монтаж
      if (na.G(i, 'dt_montage') = null) and (na.G(i, 'is_complaint') <> 1) then
        S.ConcatStrP(st, 'Монтаж не завершен', ' / ');
    end;
    if st = '' then
      ids := ids + [na.G(i, 'id')]
    else
      S.ConcatStrP(msg, na.G(i, 'ornum') + ':  ' + st, #13#10);
  end;
  if Length(ids) > 0 then begin
    for i := 0 to High(ids) do begin
      if Q.QExecSql('update orders set dt_end_manager = :dt$d where id = :id$i', [Date, ids[i]]) <> -1 then
        //автоматически попробуем завершить заказ
        FinalizeOrder(ids[i], myOrFinalizeManager, 1, True);
    end;
    Result := True;
  end;
  MyInfoMessage(S.IIFStr(Length(ids) = 0, 'Ни один заказ не завершен!', S.IIFStr(High(ids) = na.High, 'Все заказы завершены.',
    'Проставлена отметка завершщения для ' + S.GetEndingFull(Length(ids), 'заказ', 'а', 'ов', 'ов') + '.' + #13#10 + 'Следующие заказы не были завершены:')) + #13#10 + msg, 1);
end;
...
function TOrders.GetProductionOperations(AArea: Integer): TVarDynArray2;
begin
  Result := [];
  if AArea = 0 then
    Result := [['Распил',2], ['Пресс',14], ['Кромка',3], ['Сверловка',4], ['ЧПУ',5], ['Стекло',6], ['Камень',7], ['Лазер',15], ['Реклама',8], ['Сборка',9], ['Шлифовка', 16], ['Лакокраска',10], ['Электрика',11], ['Комплектовка',12], ['Упаковка',13], ['Прочие',1]];
  if AArea = 2 then
    Result := [['Раскрой',2], ['Гибка',3], ['Сварка',4], ['Зачистка',5], ['Подготовка к покраске',6], ['Покраска',7], ['Сборка',8], ['Упаковка',9], ['Прочие',1]];
end;
...
begin
  ...
  for i := 0 to High(names) do begin
    S.ConcatStP(st1, 'i' + IntToStr(names[i][1]), ', ');
    S.ConcatStP(st2, 'i' + IntToStr(names[i][1]) + ' = :i' + IntToStr(names[i][1]) + '$i', ', ');
  end;
  vin := Q.QLoadRow('select ' + st1 + ', i1 from ' + table + ' where id = :id$i and id_area = :id_area$i', [AId, AAreaId]);
  vctrls := [];
  for i := 0 to High(names) do
    vctrls := vctrls + [[cntNEdit, names[i][0], '0:100000:0']];
  ...
  Q.QExecSql('update ' + table + ' set ' + st2 + ' where id = :id$i and id_area = :id_area$i', vout + [AId, AAreaId]);
  Result := True;
end;

function TOrders.SetLaborCost(AOwner: TComponent; AAreaId: Integer): Boolean;
//ввод стоимости работ по операциям
var
  i, j: Integer;
  vin, vout: TVarDynArray;
  names: TVarDynArray2;
...
begin
  ...
  st2 := '';
  for i := 0 to High(names) do begin
    S.ConcatStP(st1, 'p' + IntToStr(names[i][1]), ', ');
    S.ConcatStP(st2, 'p' + IntToStr(names[i][1]) + ' = :p' + IntToStr(names[i][1]) + '$f', ', ');
  end;
  vin := Q.QLoadRow('select ' + st1 + ', p1 from ' + table + ' where id_area = :id_area$i', [AAreaId]);
  vctrls := [];
  for i := 0 to High(names) do
    vctrls := vctrls + [[cntNEdit, names[i][0], '0:100000:2']];
  ...
  Q.QExecSql('update ' + table + ' set ' + st2 + ' where id_area = :id_area$i', vout + [AAreaId]);
  Result := True;
end;

function TOrders.EraseOutdatedOrders(AOwner: TComponent): Boolean;
var
...
begin
  ...
  if cnt = 0 then begin
    MyInfoMessage('Ни один заказ не будет очищен.');
    Exit;
  end
  else if cnt > 1000 then
    MyWarningMessage('Слишком много заказов за данный период! Уменьшите период для очистки!')
