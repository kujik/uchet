...
begin
  ...
  Cnt := 0;
  Msg := '';
  //дл€ создани€ по всем подразделени€м
  if pgcMain.ActivePageIndex = 0 then begin
    S.GetDatesFromPeriodString(cmbPeriodD.Text, dt1, dt2);
    if MyQuestionMessage('—оздать расчетные ведомости за период ' + DateToStr(dt1) + ' - ' + DateToStr(dt2) + '?') <> mrYes then
      Exit;
    ...
    //получим список ведомостей по зп, по полным подразделени€м за этот период
    ...
    for i := 0 to High(va1) do begin
      if A.PosInArray(va1[i][0], va2, 0) = -1 then begin
        ...
        if va1[i][2] <> 1 then
          //уведомление о незакрытых “”–¬
          S.ConcatStP(Msg, va1[i][1], #13#10);
        //создаем зарплатную ведомость, если все же во врем€ между проверками уже така€ была создана, то будет ошибка уникального индекса, здесь ее не выводим
        if Integer(va1[i, 0]) in [4,5] then  //!!! »“, бухгалтери€
    ...
    //сообщение
    if Msg <> '' then
      Msg := #13#10#13#10'ѕо следующим подразделени€м не закрыты “”–¬:'#13#10 + Msg;
  end
  //дл€ создани€ по выбранному работнику
  else begin
    S.GetDatesFromPeriodString(cmbPeriodW.Text, dt1, dt2);
    if MyQuestionMessage('—оздать расчетные ведомости за период ' + DateToStr(dt1) + ' - ' + DateToStr(dt2) + #13#10'дл€ работник "' + cmbWorker.Text + '"?') <> mrYes then
      Exit;
    //получм список ведомостей по уволенным, которые нужно создать
    //дл€ каждого периода прием-увольнение будет создано не меньше одной ведомости, едина ведомость на несколько таких периодов никогда не создаетс€.
    //(если были переходы в пределах такого периода в разныве подразделдени€, то создастс€ по каждому подразделению)
    Q.QLoadFromQuery(
  ...
  //сообщение
  if Cnt = 0 then
    Msg := 'Ќи одна ведомость не создана!' + Msg
  else
    Msg := '—оздан' + S.GetEnding(cnt, 'а', 'о', 'о') + ' ' + IntToStr(cnt) + ' ведомост' + S.GetEnding(cnt, 'ь', 'и', 'ей') + '.' + Msg;
  MyInfoMessage(Msg, 1);
...
begin
  Result := False;
  Caption := '~ƒобавить расчетные ведомости';
