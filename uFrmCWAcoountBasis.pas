...
//      'select nvl(prc,0) ' + S.IIf((pgc_Main.ActivePage = ts_Orders), 'id_order', 'id_acc') + ', '+
    ...
    if maxprc >= 100 then begin
      MyInfoMessage('Эта запись не может быть внесена в счет в качестве основания, так как уже полностью внесена в другие счета!' + st, 1);
      b := False;
    end
    else if (maxprc > 0) and (prc > 100 - maxprc) then begin
      MyInfoMessage('Эта запись может быть внесена в счет в качестве основания, при условии выставления процента не более' + IntToStr(100 - maxprc) + '!' + st, 1);
      b := False;
