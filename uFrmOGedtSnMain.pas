...
type
  ...
  private
    ...
    function  ExportGridToXLSXnew(Open: Boolean = False; OnlyNot0: Boolean = False): Boolean;
...
begin
  ...
  FFields := [
    ...
    ['artikul$s','Номенклатура|Артикул','80','t=x'],
    ['name$s','Номенклатура|Наименование','180','t=x'],
    ['name_unit$s','Номенклатура|Ед.изм.','50','t=x'],
    ['in_estimates$s','Номенклатура|Смета','25', 'pic=+;-:0;6'],
    ['has_files$s','Номенклатура|Файлы.','40','pic'],
    ['supplierinfo$s','Поставщик|Наименование','120','bt=Поставщики','t=x,2'],
    ['suppliers_cnt$s','Поставщик|Кол.','40','t=x,2'],
    ['no_namenom_supplier$s','Поставщик|Нет назв.','40','t=2','pic=-:6'],
    ['qnt0$f','Расход|'+S.GetDaysCountToName(FDays[0]),'60','t=x']
  ];
  //Q.QExecSql('selet 1 from', [], False);
  va := Copy(FPeriodNames, 1);
  //поля по периодам для прихода
  for i:=1 to High(va) + 1 do
    FFields := FFields + [['qnti' + InttoStr(i) + '$f', 'Приход|' + va[i - 1], '60', 't=1']];
  //поля по периодам для расхода
  for i:=1 to High(va) + 1 do
    FFields := FFields + [['qnt' + InttoStr(i) + '$f', 'Расход|' + va[i - 1], '60', 't=1']];
  FFields := FFields +
  [
    ['qnta0$f','Текущий остаток|ПЩ','60'],
    ['qnta1$f','Текущий остаток|ЛОК','60'],
//    ['qnta2$f','Текущий остаток|ДМ','60'],
    ['qnt$f','Текущий остаток|Всего','60','t=x'],
    ['prc_qnt$f','% по остатку','40','t=x'],
    ['rezerva0$f','Текущий резерв|ПЩ','40'],
    ['rezerva1$f','Текущий резерв|ЛОК','40'],
//    ['rezerva2$f','Текущий резерв|ДМ','40'],
    ['rezerv$f','Текущий резерв|Всего','40','t=x'],
    ['qnt_onway$f','В пути|Кол-во','40','t=x'],
    ['onway_old$f','В пути|Давно','40', 'pic=1:3'],
    ['qnt_onway_surplus$f','В пути|Превы-'#13#10'шение','50'],
    ['qnt_in_processing$f','В обработке','40'],
    ['qnt_suspended$f','Зависшие остатки','40'],
    ['min_ostatok$i','Минимальный остаток|ВВОД','40','t=x,3'],
    ['prc_min_ost$f','Минимальный остаток|%','40'],
    ['qnt_order_opt$i','Партия ВВОД','40','t=3'],
    ['planned_need_days$i','Период закупки  ВВОД','40','t=3'],
    ['qnt_order$f','Заказ|ВВОД','60','t=x,3'],
    ['to_order$i','Заказ|В заявку','40', 'chb','f=f:','t=x,3'],
    ['need$f','Потребность|Мин.','60','t=x'],
    ['need_p$f','Потребность|Плановая','60'],
    ['need_m$f','Потребность|С остатком','60','t=x'],
    ['prc_need_m$f','Потребность|%','40'],
    ['price_check$f','Стоимость|Контрольная цена','60','t=x'],
    ['monitor_price$i','Стоимость|Мониторинг цены','60','chb','t=3'],
    ['price_main$f','Стоимость|Цена','60','t=x'],
    ['price_diff$s','Стоимость|Цена, +/-','40'],
    ['order_cost$f','Стоимость|Заказ','60','f=r:','t=x'],
...
//    ['qnt_in_processing','количество по позиции, по кторой созданы заявки на снабжение, за минусом уже оприходованного количества и за минусом количества В пути. По даблклику - дополнительная информация.'],
    ...
    ['qnt_onway_surplus','Эта ячейка показывает превышение сырья, находящегося в пути, над требуемым резервом по заказам. Если превышения нет, то она останется пустой.'],
    ['','']
  ];
...
begin
  if Tag = mbtTest then begin
    ...
    ExportGridToXLSXnew(True, False);
    Handled := True;
  end
...
var ExportToXlsxFormatterBgr: TVarDynArray2;
var ExportToXlsxFormatterClr: TVarDynArray2;
procedure ExportToXlsxFormatter(var Fr: TFrDBGridEh; FieldName: string; Params: TColCellParamsEh);
//callback для форматирования файла эксель
begin
  Params.Background := ExportToXlsxFormatterBgr[Fr.RecNo - 1][FrmOGedtSnMain.Frg1.DBGridEh1.FindFieldColumn(FieldName).Index];
  Params.Font.Color := ExportToXlsxFormatterClr[Fr.RecNo - 1][FrmOGedtSnMain.Frg1.DBGridEh1.FindFieldColumn(FieldName).Index];
end;

function TFrmOGedtSnMain.ExportGridToXLSXnew(Open: Boolean = False; OnlyNot0: Boolean = False): Boolean;
begin
ExportGridToXLSX(Open, OnlyNot0); exit;
  ForceDirectories(Module.GetPath_Demand_Created);
  var LFileName := Module.GetPath_Demand_Created +'\' + FormatDateTime('yyyy-mm-dd_hh.nn.ss', Now) + '__(' + User.GetName + ')' + '.xlsx';
  var LFields: TVarDynArray2 := [];
  var LFieldNames := '';
  for var i := 0 to High(FFields) do
    for var j := 0 to High(FFields[i]) do
      if Pos('t=x', VarToStr(FFields[i][j])) > 0 then begin
        LFields := LFields + [FFields[i]];
        S.ConcatStP(LFieldNames, FFields[i][0], ';');
        Break;
      end;
  ExportToXlsxFormatterBgr := [];
  ExportToXlsxFormatterClr := [];
  for var k := 1 to Frg1.RecordCount do begin
    Frg1.RecNo := k;
    Application.ProcessMessages;
    ExportToXlsxFormatterBgr := ExportToXlsxFormatterBgr + [FrmOGedtSnMain.FRowColors];
    ExportToXlsxFormatterClr := ExportToXlsxFormatterClr + [FrmOGedtSnMain.FRowFontColors];
  end;
  var LDate: TNamedArr := Frg1.ExportToNa(LFieldNames, False);
  ExportToXlsx(LFileName, LDate, LFields, 'Выгрузка от ' + DateTimeToStr(Now), '', True, ExportToXlsxFormatter);
  if Open then
    Sys.ExecFileIfExists(LFileName, 'Файл не найден!');
end;
...
begin
  ...
  if MyQuestionMessage('Сохранить текущее состояние данных в Excel-файле?') = mrYes then begin
    ...
    ExportGridToXLSXnew(False);
    //восстановим фильтр
    Gh.GridFilterRestore(Frg1.DbGridEh1, gf);
  end;
  //в рассылку информацию по заявке
  SendMail;
