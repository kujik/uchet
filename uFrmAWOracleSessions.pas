...
type
  TFrmAWOracleSessions = class(TFrmBasicGrid2)
    Timer1: TTimer;
    BtCloseModules: TBitBtn;
    NeIdle: TDBNumberEditEh;
    procedure Timer1Timer(Sender: TObject);
    procedure BtCloseModulesClick(Sender: TObject);
  private
...
begin
  ...
  for i := 0 to cMainModulesCount - 1 do begin
    c:=Cth.CreateControls(PMDIClient, cntCheck, ModuleRecArr[i].Caption, 'Chb' + IntToStr(i), '', 0);
    c.Left := 4;
    c.Width := PLeft.Width - 10;
    c.Top := t + 4 + i * MY_FORMPRM_CONTROL_H;
  end;
  Cth.SetBtn(BtCloseModules, mybtClose, False, 'ѕослать сигнал завершени€');
end;

procedure TFrmAWOracleSessions.BtCloseModulesClick(Sender: TObject);
var
  i, j: Integer;
  Cb: TCheckBox;
begin
  if NeIdle.Value = null then
    exit;
  ...
  for i:= 0 to 15 do begin
    ...
    if not Cb.Checked then
      Continue;
    Q.QExecSql('update adm_modules set autoclosedt = (select sysdate from dual), autoclosemin = :idle$i where id = :id$i', [NeIdle.Value, i]);
  end;
end;
...
procedure TFrmAWOracleSessions.Timer1Timer(Sender: TObject);
begin
  inherited;
  Frg1.RefreshGrid;
  GetInfo;
