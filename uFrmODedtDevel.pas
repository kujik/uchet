...
type
  TFrmODedtDevel = class(TFrmBasicDbDialog)
    ...
    nedt_hours_total: TDBNumberEditEh;
  private
    FDeveloper: Integer;
    FCaption: string;
...
begin
  ...
  F.DefineFields:=[
    ...
    ['hours_total$f;0','V=0:9999999:1',#0],
  ...
  FOpt.InfoArray := [[
    ...
    'Поля № изделия, Количество изделий, Исполнитель и Комментарий обязательными не являются.'#13#10
  ]];
  FWHBounds.X := 610;
  FWHBounds.Y := 400;
  Result := inherited;
  if not Result then
    Exit;
  SetControlsEditable([dedt_Dt_Beg, nedt_Hours, nedt_hours_total], False);
...
begin
  if Sender = cmb_Id_DevelType then begin
    if cmb_Id_DevelType.Value.AsString = '' then begin
      Cth.SetControlValue(nedt_Hours, null);
      Cth.SetControlValue(nedt_Hours_total, null);
    end
    else begin
      Cth.SetControlValue(nedt_Hours, FHours[A.PosInArray(cmb_Id_DevelType.Value, FHours, 1)][0]);
      Cth.SetControlValue(nedt_Hours_total, nedt_Hours.Value * Max(1, nedt_qnt.Value.AsInteger));
    end;
  end;
  if Sender = nedt_qnt then begin
    Cth.SetControlValue(nedt_Hours_total, nedt_Hours.Value * Max(1, nedt_qnt.Value.AsInteger));
  end;
