...
begin
  ...
  for i := 0 to High(va) do begin
    ...
    Q.QLoad('select id_schedule, hours, hours1, hours2, null as period_hours, approved from w_schedule_periods where id_schedule = :id$i and dt = :dt$d', [va[i], FDtMonth], FSchedules[i].Title);
    ...
    if b then begin
      ...
      FSchedules[i].Hours := Q.QLoad('select hours, dt from w_schedule_hours where id_schedule = :id$i and dt >= :dt1$d and dt <= :dt2$d order by dt', [va[i], FDtBeg, FDtEnd]);
...
begin
  FTurvCodeW := -1;
  Q.QLoad('select id, code, name from ref_turvcodes order by code', [], FTurvCodes);
...
begin
  ...
  id_vyh:=Q.QLoadValue('select id from w_turvcodes where code = ''В''', []);
...
//          vt[i][7]:=1;
  ...
  for i := 0 to High(FRows) do begin
    ...
    for d := 1 to FDaysCount do begin
      ...
      st := Q.QGetSql('Q', 'w_turv_day', 'begtime$f;endtime$f;settime3$i;id_turvcode2$i;worktime2$f' + ' where id_employee_properties = :ide$i and dt = :dt$d');
...
begin
  ...
  vaTable := Q.QLoad(
    Q.QGetSql('a', 'w_turv_export', Flds) + ' where id_departament = :id_departament$i and dt1 = :dt1$d',
...
begin
  Result:=Q.QLoad('select dt, status, id_division, id_job from j_worker_status where id_worker = :id$i order by dt desc, id_division, id_job', [id]);
end;
...
begin
  ...
  //запрос. сортировка принципиальна
  v:=Q.QLoad('select id_division, id_worker, workername, id_job, job, status, dt, id_schedule from v_j_worker_status order by workername, dt, job, id_division',[]);
...
begin
  ...
  case AllDivisions of
    1: begin
        v0:=Q.QLoad('select id, name, active, editusers, id_schedule from v_ref_divisions where 1 = 1' + st, []);
       end;
    2: begin
        v0:=Q.QLoad('select id, name, active, editusers, id_schedule from v_ref_divisions where IsStInCommaSt(:u$i, editusers) = 1' + st, [User.GetId]);
       end;
    else
      begin
        v0:=Q.QLoad(
  ...
  for i:= 0 to High(v0) do begin
    ...
    Q.QBeginTrans;
    res:=Q.QSave('i', 'turv_period', '', 'id$i;id_division$i;dt1$d;dt2$d;commit$i;id_schedule$i', [0, v0[i][0], dt, GetTurvEndDate(dt), 0, v0[i][4]]);
    if Q.QRowsAffected = 0 then Result:=-1;
    if res = -1 then Result:=-1;
    if Result =- 1 then Break;
    for j:=0 to High(v2) do begin
      res2:=Q.QSave('i', 'turv_worker', '', 'id$i;id_turv$i;id_worker$i;id_division$i;id_job$i;dt1$d;dt2$d;dt1p$d;dt2p$d;id_schedule',
        [
          0,
...
//  v0:=v0 + v1;
  ...
  if ID_Division <> null then begin
    st := Q.QGetSql('A', 'turv_day', dayfields) + ' where id_worker = :id_worker$i and dt >= :dt$d order by dt';
    days := Q.QLoad(st, [Id_Worker, Dt]);
  end;
  Result := True;
  Result1 := False;
  //начнем транзакцию
  ...
  try
    ...
    //изменим таблицу статусов, тк она понадобится для коррекции периодов
      if Mode = -1 then begin
        v3 := [ID_JStatus];
//        Result:= (Q.QSave('d', 'j_worker_status', 'sq_j_worker_status', 'id$i;name;office;id_head;editusers', v3) >= 0);
        Result := (Q.QSave('d', 'j_worker_status', 'sq_j_worker_status', 'id$i', [ID_JStatus]) >= 0);
      end
      else begin
        v3 := [ID_JStatus, Id_Worker, ID_Division, ID_Job, Mode, Dt];
        Result := (Q.QSave('i', 'j_worker_status', 'sq_j_worker_status', 'id$i;id_worker$i;id_division$i;id_job$i;status$i;dt$d', [ID_JStatus, Id_Worker, ID_Division, ID_Job, Mode, Dt]) >= 0);
      end;
      if not Result then
        Break;
    ...
    //снова проставим данные по дням работника, для нового айди и тех же дат
      for i := 0 to High(days) do begin
        ...
        st := Q.QGetSql('Q', 'turv_day', dayfields) + ' where id_worker = :id_worker$i and dt = :dt_day$d';
...
//      if (v1[i][9] >= 2)and(v1[i][2] = v2[j][2])and(v1[i][4] = v2[j][4])and(v1[i][7] = v2[j][0]) then begin
    ...
    if not b then begin
      Result:=Q.QSave('i', 'turv_worker', '', 'id$i;id_turv$i;id_worker$i;id_division$i;id_job$i;dt1$d;dt2$d;dt1p$d;dt2p$d',
        [
          0,
...
begin
  Result:=Q.QLoad('select id, workername, dt, divisionname, job from v_ref_workers where job is not null', []);
end;
...
//  dt1:=GetTurvBegDate(IncDay(GetTurvBegDate(Date), -1));
  ...
  //вторая дата - конец текущих суток
  dt2:=IncMilliSecond(IncDay(Date, 1), -1);
  vdiv:=Q.QLoad('select id from ref_divisions where office = 1', []);
  id_vyh:=Q.QLoadValue('select id from ref_turvcodes where code = ''В''', []);
  if id_vyh = null then id_vyh:=-1;
  id_otp:=Q.QLoadValue('select id from ref_turvcodes where code = ''ОТ''', []);
...
//    '(begtime is null or endtime is null) and '+
  ...
  vw:=Q.QLoad('select id, workername from v_ref_workers order by id', []);
  //массивы данных по входу и уходу работников
  vpb:=[];
  vpe:=[];
...
begin
  Result:=[];
  va2:=Q.QLoad('select dt, type from ref_holidays where dt >=:dt1$d and dt <=:dt2$d', [DtBeg, DtEnd]);
  dt:=DtBeg;
...
begin
  ...
  Id := Q.QSave(Q.QFModeToIUD(AMode), 'w_schedules', '', 'id$i;code$s;name$s;comm;active$i', [AId, va13[0], va13[1], va13[6], va13[7]]);
  if Id = -1 then
    Exit;
  if AMode = fAdd then
    AId := Id;
  for i := 0 to 3 do begin
    ...
    Id := Q.QSave(S.IIf(IdH = null, 'i', 'u'), 'w_schedule_hours', '', 'id$i;id_schedule$i;dt$d;hours$f', [IdH, AId, va11[i], va13[1 + i]]);
  end;
  Result := True;
end;
...
begin
  ...
  if MyQuestionMessage('Создать все ТУРВ за период с ' + DateToStr(Turv.GetTurvBegDate(LDate)) + ' по ' + DateToStr(Turv.GetTurvEndDate(LDate)) + ' ?') <> mrYes then
    Exit;
  va1 := Q.QLoadCol('select distinct id_departament from w_employee_properties where dt_beg <= :dte$d and (dt_end is null or dt_end >= :dtb$d)', [Turv.GetTurvEndDate(LDate), Turv.GetTurvBegDate(LDate)]);
  va2 := Q.QLoadCol('select distinct id_departament from w_turv_period where dt1 = :dt$d', [Turv.GetTurvBegDate(LDate)]);
  va3 := Q.QLoadCol('select id from w_departaments where active = 1', []);
  j := 0;
...
begin
  ...
  //вторая дата - конец текущих суток
  dt2:=IncMilliSecond(IncDay(Date, 1), -1);
  vdiv:=Q.QLoad('select id from w_departaments where is_office = 1', []);
  id_vyh:=Q.QLoadValue('select id from w_turvcodes where code = ''В''', []);
  if id_vyh = null then id_vyh:=-1;
  id_otp:=Q.QLoadValue('select id from w_turvcodes where code = ''ОТ''', []);
  if id_otp = null then id_otp:=-1;
...
{  if TFrmBasicInput.ShowDialog(Application, '', [], fAdd, '~Создать ведомости', 185, 45, [[cntComboL,'Период','1:400:0', 135]],
  ...
  if (Q.QLoadValue('select count(id) from w_advance_calc where id_employee is null and dt = :dt$d', [dt1]) <> 0) and
    ...
    then begin
      if Q.QSave('i', 'w_advance_transfer', '', 'id$i;dt$d', [0, dt1]) <> -1 then
        inc(Cnt);
    end;
  //ведомости по уволенным работникам - сгруппированы по работнику и периоду работы
  va1 := Q.QLoad('select id_employee, personnel_number from w_advance_calc where id_employee is not null and dt = :dt$d group by id_employee, personnel_number', [dt1]);
  va2 := Q.QLoad('select id_employee, personnel_number from w_advance_transfer where id_employee is not null and dt = :dt$d', [dt1]);
    for i := 0 to High(va1) do begin
      if (A.PosRowInArray(va1, va2, i) = -1) then begin
        //если ведомость еще не создана
        if Q.QSave('i', 'w_advance_transfer', '', 'id$i;id_employee$i;personnel_number$s;dt$d', [0, va1[i][0], va1[i][1], dt1], False) <> -1 then
          inc(Cnt);
      end;
    end;
  //сообщение
...
begin
  ...
  va1 := Q.QLoad('select id_departament from w_advance_calc where id_employee is null and dt = :dt$d', [dt1]);
  va2 := Q.QLoad('select id_departament from w_advance_cash where id_employee is null and dt = :dt$d', [dt1]);
  for i := 0 to High(va1) do begin
    if (A.PosRowInArray(va1, va2, i) = -1) then begin
      if Q.QSave('i', 'w_advance_cash', '', 'id$i;id_departament;dt$d', [0, va1[i][0], dt1], False) <> -1 then
  ...
  va1 := Q.QLoad('select id_departament, id_employee, personnel_number from w_advance_calc where id_employee is not null and dt = :dt$d', [dt1]);
  va2 := Q.QLoad('select id_departament, id_employee, personnel_number from w_advance_cash where dt = :dt$d', [dt1]);
  va3 := Q.QLoad('select id_departament, id_employee, personnel_number from w_employee_properties where is_terminated = 1 and dt_beg >= :dt1$d and dt_beg <= :dt2$d', [dt1,  dt3]);
  for i := 0 to High(va1) do begin
    if (A.PosRowInArray(va1, va2, i) = -1) and (A.PosRowInArray(va1, va3, i) > -1) then begin
      ...
      if Q.QSave('i', 'w_advance_cash', '', 'id$i;id_departament;id_employee$i;personnel_number$s;dt$d', [0, va1[i][0], va1[i][1], va1[i][2], dt1], False) <> -1 then
        inc(Cnt);  //увеличим количество созданных
    end;
  end;
  //сообщение
  ...
  S.GetDatesFromPeriodString(va[0], dt1, dt2);
{
  ...
  //по уволенным захватываем начало следующего периода, так как по ним могли быть сформироываны ведомости уже в текущем периоде - иначе по ним ведомсти к выдааче не создадутся!
//  va3 := Q.QLoad('select id_departament, id_employee, id_organization, personnel_number from w_employee_properties where is_terminated = 1 and dt_beg >= :dt1$d and dt_beg <= :dt2$d', [dt1, Turv.GetTurvEndDate(IncDay(dt2))]);
  va3 := Q.QLoad('select id_departament, id_employee, id_organization, personnel_number from w_employee_properties where is_terminated = 1 and dt_beg >= :dt1$d and dt_beg <= :dt2$d', [dt1, EndOfTheMonth(IncDay(dt2, 1))]);
...
begin
  ...
  LIds := Q.QLoadCol('select id from w_turv_period where is_finalized = 0 and dt1 = :dt1$d', [GetTurvBegDate(IncDay(GetTurvBegDate(Date), -1))]);
  LIds := LIds + Q.QLoadCol('select id from w_turv_period where is_finalized = 0 and dt1 = :dt1$d', [GetTurvBegDate(Date)]);
...
begin
  ...
  va := Q.QLoadCol('select id from w_turv_period where dt1 = :dt$d and is_finalized = 0', [Turv.GetTurvBegDate(LDate)]);
...
Exit;
  ...
  Q.QLoad('select 0 as f, id, id_employee, id_job, id_departament, is_hired, is_terminated, dt_beg, dt_end, personnel_number, id_organization, is_concurrent, id_schedule from w_employee_properties order by id_employee, id desc', [], es);
  w := Q.QLoad('select  id, personnel_number, id_organization, concurrent_employee from ref_workers', []);
  sh := Q.QLoad('select * from (select id_worker, id_schedule_active, dt1, row_number() over (partition by id_worker order by id desc) as rn from v_turv_workers) where rn = 1' ,[]);
...
//      es.SetValue(i, 'is_concurrent', w[j][3]); //!!!
  ...
  for i := 0 to es.Count - 1 do
    if es.G(i, 'f') = 1 then
      Q.QSave('u', 'w_employee_properties', '', 'id$i;id_employee$i;id_job$i;id_departament$i;is_hired$i;is_terminated$i;dt_beg$d;dt_end$d;personnel_number$s;id_organization$i;is_concurrent$i;id_schedule$i',[
