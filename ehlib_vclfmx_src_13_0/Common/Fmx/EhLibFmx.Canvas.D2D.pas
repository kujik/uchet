...
{$ENDIF}
  ...
  public
    ...
    class property OnDeviceRemoved: TD2DDeviceRemoveEvent read FOnDeviceRemoved write FOnDeviceRemoved;
  end;

procedure RegisterCanvasClasses;
procedure UnregisterCanvasClasses;
procedure UseEhLibDirect2DCanvas();
...
type
  ...
  public
    ...
    property SavedClipPushed: Boolean read FSavedClipPushed write FSavedClipPushed;
  end;
...
{$ENDIF}
  ...
  private type
    TFontDescriptor = record
      ...
      CustomFontCollection: IDWriteFontCollection;
...
{$IFDEF EH_LIB_29} 
    { Font managment }
    class function GetDWriteLocalizedName(const Names: IDWriteLocalizedStrings; const LocaleName: string): string;

    class function TryGetFontDescriptorFromCustomFontCollection(const AFont: TFont; var ADescriptor: TFontDescriptor): Boolean;
    class function TryGetFontDescriptorFromGDI(const AFont: TFont; var ADescriptor: TFontDescriptor): Boolean;
{$ELSE}
{$ENDIF}
    class function GetFontStyles(const AFont: TFont): TFontDescriptor;
  private
...
{$IFDEF EH_LIB_29} 
class constructor TCustomBitmapCodecWIC.Create;
begin
  FFactoryLock := TObject.Create;
end;

class destructor TCustomBitmapCodecWIC.DestroySharedResources;
begin
  FImagingFactory := nil;
  FreeAndNil(FFactoryLock);
end;
{$ELSE}
{$ENDIF}
...
function CoInitializeNotInMainThread: Boolean;
begin
  if TThread.Current.ThreadID = MainThreadID then
    Result := False
  else
    Result := CoInitializeEx(nil, COINIT_APARTMENTTHREADED) = S_OK;
end;

procedure CoUninitializeNotInMainThread;
begin
  if TThread.Current.ThreadID <> MainThreadID then
    CoUninitialize;
end;
...
var
  ...
  W, H: UINT;
  ComInitResult: Boolean;
begin
  ComInitResult := CoInitializeNotInMainThread;
  try
    W := 0;
    H := 0;
    ...
    if ComInitResult then
      CoUninitializeNotInMainThread;
  end;
end;
...
function TBitmapCodecWIC.LoadFromFile(const AFileName: string; const Bitmap: TBitmapSurface; const MaxSizeLimit: Cardinal): Boolean;
var
  Decoder: IWICBitmapDecoder;
  Frame: IWICBitmapFrameDecode;
  Stream: TStream;
...
begin
  ...
  try
    ...
    try
      ...
      ImagingFactory.CreateStream(Stream);
      Stream.InitializeFromMemory(CopyStream.Memory, CopyStream.Size);
      ImagingFactory.CreateDecoderFromStream(Stream, GUID_NULL, WICDecodeMetadataCacheOnLoad, Decoder);
      if Decoder <> nil then
      begin
        Decoder.GetFrame(0, Frame);
        if Frame <> nil then
...
{$IFDEF EH_LIB_37}
constructor TCanvasD2D.CreateFromWindow(const AParent: TWindowHandle; const AWidth, AHeight: Single;
...
{$IFDEF EH_LIB_28} 
function TCanvasD2D.DoBeginScene(AClipRects: PClipRects = nil; AContextHandle: THandle = 0): Boolean;
{$ELSE}
...
{$IFDEF EH_LIB_37}
procedure TCanvasD2D.SetSize(const AWidth, AHeight: Single);
{$ELSE}
...
begin
  ...
  begin
    ...
    FClipPushed := True;
    FClipPushedCount := FClipPushedCount + 1;
  end;
end;
...
begin
  ...
  begin
    ...
    if (AStroke.Dash = TStrokeDash.Custom) and (Length(AStroke.DashArray) > 0) then
...
var
  ...
  FontDescriptor: TTextLayoutD2D.TFontDescriptor;
begin
  ...
  FontDescriptor := TTextLayoutD2D.GetFontStyles(FFont);
  if Succeeded(SharedDWriteFactory.CreateTextFormat(PChar(FontDescriptor.FamilyName), FontDescriptor.CustomFontCollection,
                                                    FontDescriptor.Weight, FontDescriptor.Style, FontDescriptor.Stretch,
                                                    FFont.Size, PChar(FLocaleName), TextFormat)) then
  begin
...
begin
  ...
  FSharedTexture := nil;
  inherited DestroySharedResources;
end;
...
{$IFDEF EH_LIB_29} 
    function DrawInlineObject(clientDrawingContext: Pointer; originX: Single; originY: Single;
      const inlineObject: IDWriteInlineObject; isSideways: BOOL; isRightToLeft: BOOL;
...
begin
  ...
  begin
    ...
    FSavedClipPushed := LSource.FClipPushed;
    LSource.FClipPushed := False;
  end;
end;

procedure TD2DCanvasSaveState.AssignTo(Dest: TPersistent);
var
  DestCanvas: TCanvasD2D;
begin
  if Dest is TCanvasD2D then
  begin
    DestCanvas := TCanvasD2D(Dest);
    ...
    DestCanvas.FClipPushed := FSavedClipPushed;
  end;
...
{$IFDEF EH_LIB_29} 
class function TTextLayoutD2D.GetDWriteLocalizedName(const Names: IDWriteLocalizedStrings; const LocaleName: string): string;
const
  DefaultLocale = 'en-US';
var
...
class function TTextLayoutD2D.GetFontStyles(const AFont: TFont): TFontDescriptor;
var
...
begin
  ...
  { Looking font in system default font collection }
  ...
  begin
    ...
    begin
      { Looking font in custom font collection }
      if TryGetFontDescriptorFromCustomFontCollection(AFont, FontDescriptor) then
        Result := FontDescriptor
      else if TryGetFontDescriptorFromGDI(AFont, FontDescriptor) then
        Result := FontDescriptor;
      ...
      try
        ...
        try
          ...
          try
            ...
            try
              Result := TFontDescriptor.Create(GetDWriteLocalizedName(Names, FLocaleName), DWriteFont);
              FFontDescriptors.Add(AFont.Family, Result);
            finally
              Names := nil;
            end;
...
begin
  ...
  if Succeeded(TCanvasD2D.SharedDWriteFactory.CreateTextFormat(PChar(FontDescriptor.FamilyName), FontDescriptor.CustomFontCollection,
                                                               FontDescriptor.Weight, FontDescriptor.Style,
                                                               FontDescriptor.Stretch, Font.Size, PChar(FLocaleName),
                                                               TextFormat)) then
  try
    if RightToLeft then
      TextFormat.SetReadingDirection(DWRITE_READING_DIRECTION_RIGHT_TO_LEFT)
...
{$IFDEF EH_LIB_29} 
class function TTextLayoutD2D.TryGetFontDescriptorFromCustomFontCollection(const AFont: TFont;
  var ADescriptor: TFontDescriptor): Boolean;
var
  Collection: IDWriteFontCollection;
  FamilyIndex: Cardinal;
  FamilyExists: LongBool;
  FontCollectionFactory: IFontCollectionFactoryDW;
begin
  if TFontManager.HasCustomFonts and Supports(TFontManager.Service, IFontCollectionFactoryDW, FontCollectionFactory) then
  begin
    Collection := FontCollectionFactory.CreateCustomFontCollection;

    if Collection.GetFontFamilyCount > 0 then 
    begin
      Collection.FindFamilyName(PChar(AFont.Family), FamilyIndex, FamilyExists);
      if FamilyExists then
      begin
        ADescriptor := TFontDescriptor.Create(AFont);
        ADescriptor.CustomFontCollection := Collection;
        Result := True;
      end
      else
        Result := False;
...
class function TTextLayoutD2D.TryGetFontDescriptorFromGDI(const AFont: TFont; var ADescriptor: TFontDescriptor): Boolean;
var
  DWriteGdiInterop: IDWriteGdiInterop;
  LogFont: TLogFont;
  DWriteFont: IDWriteFont;
...
begin
  ...
  if 
    (TCustomCanvasD2D.Direct3DLevel >= TCustomCanvasD2D.TDirect3DLevel.Direct3D10) and
    InitThemeLibrary and
    UseThemes and
    TCustomCanvasD2D.TryCreateDirect3DDevice then
  begin
