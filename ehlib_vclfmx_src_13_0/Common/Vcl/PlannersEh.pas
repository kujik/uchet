...
uses
  SysUtils, Messages, Forms, Controls, StdCtrls, TypInfo,
...
{$ENDIF}
  {$IFDEF FPC}
    ...
    {$ENDIF}
  {$ELSE}
    EhLibVclUtils, PrintUtilsEh, Windows, UxTheme,
  {$ENDIF}
...
{ TDrawSpanItemArgsEh }
  ...
  TSpanInteractiveChange = (sichSpanLeftSizingEh, sichSpanRightSizingEh,
    sichSpanTopSizingEh, sichSpanBottomSizingEh, sichSpanMovingEh);
...
{ TTimeSpanDisplayItemEh }
  ...
  public
    ...
    property StopGridRolPos: Integer read FStopGridRolPos;
...
{ TPlannerViewSelectedRangeEh }
  ...
  TPlannerStateEh = (psNormalEh, psSpanLeftSizingEh, psSpanRightSizingEh,
    psSpanTopSizingEh, psSpanBottomSizingEh, psSpanMovingEh, psSpanTestMovingEh,
...
{ TCustomPlannerViewEh }
  ...
  protected
    ...
    procedure ResetDayNameFormat(LongDayFactor, ShortDayFactor: Double); virtual;
    procedure ResetGridControlsState; virtual;
    procedure ResetLoadedTimeRange;
    procedure ResetResourcesViewArray; virtual;
  ...
  public
    ...
    procedure DrawSpanItemBackground(SpanItem: TTimeSpanDisplayItemEh; const ARect: TRect; DrawArgs: TDrawSpanItemArgsEh); virtual;
...
{$ENDIF}
  ...
  TAxisTimeBandOrientationEh = (atboVerticalEh, atboHorizontalEh);

{ TAxisTimeBandPlannerViewEh }

  TPlannerAxisTimelineViewEh = class(TCustomPlannerViewEh)
  private
    FBandOrientation: TAxisTimeBandOrientationEh;
    FDatesBarArea: TDatesBarAreaEh;
    FRangeKind: TTimePlanRangeKind;

    function GetSortedSpan(Index: Integer): TTimeSpanDisplayItemEh;

    procedure SetBandOrientation(const Value: TAxisTimeBandOrientationEh);
    procedure SetDatesBarArea(const Value: TDatesBarAreaEh);
    procedure SetRangeKind(const Value: TTimePlanRangeKind);
  ...
  protected
    ...
    property BandOrientation: TAxisTimeBandOrientationEh read FBandOrientation write SetBandOrientation;
    property DatesBarArea: TDatesBarAreaEh read FDatesBarArea write SetDatesBarArea;
    property RangeKind: TTimePlanRangeKind read FRangeKind write SetRangeKind;
    property SortedSpan[Index: Integer]: TTimeSpanDisplayItemEh read GetSortedSpan;
...
{ TPlannerToolBoxEh }
  ...
  {$ENDIF}
    ...
    property PlannerControl: TPlannerControlEh read GetPlannerControl;
    property NextPeriodButton: TSpeedButtonEh read FNextPeriodButton;
    property PeriodInfo: TLabel read FPeriodInfo;
    property PriorPeriodButton: TSpeedButtonEh read FPriorPeriodButton;
  end;
...
{ TPlannerDrawStyleEh }
  ...
  public
    ...
    function GetActualTodayFrameColor: TColor; virtual;
...
var
  LineItemRect: TRect;
begin
  Canvas.Brush.Color := ApproachToColorEh(PlannerDrawStyleEh.GetActualTodayFrameColor, Canvas.Brush.Color, 66);
  ...
  begin
    ...
    Canvas.Brush.Color := PlannerDrawStyleEh.GetActualTodayFrameColor;
    LineItemRect := ATimeLineRect;
...
begin
  ...
  SprCellValue := CellValues[ACol, ARow];

  ATime := VarToDateTime(SprCellValue);
  ...
  TimeCellDrawArgs.Time := VarToDateTime(SprCellValue);
...
var
  CellDate: TDateTime;
begin
  DrawArgs.Value := CellValues[ACol, ARow];
  DrawArgs.Text := '';
...
const
  SideStates: array[TRectangleSideEh] of TPlannerStateEh =
    (psSpanLeftSizingEh, psSpanRightSizingEh, psSpanTopSizingEh, psSpanBottomSizingEh);
var
  HitSpanItem: TTimeSpanDisplayItemEh;
...
begin
  ...
  if FPlannerState in
      [psSpanLeftSizingEh, psSpanRightSizingEh, psSpanBottomSizingEh,
...
begin
  ...
  if FPlannerState in [psSpanLeftSizingEh, psSpanRightSizingEh,
      psSpanTopSizingEh, psSpanBottomSizingEh, psSpanMovingEh,
      psCellsRangeSelectionEh]
  then
    ...
    begin
      ...
      begin
        ...
        Result := True;
                  ...
                  (sichSpanBottomSizingEh in CurSpanItem.AllowedInteractiveChanges) then
      begin
        Side := rsBottomEh;
        SpanItem := CurSpanItem;
        Result := True;
...
procedure TCustomPlannerViewEh.ResetDayNameFormat(LongDayFactor, ShortDayFactor: Double);
var
  ColWidth: Integer;
  i: Integer;
  Dname: String;
...
begin
  ...
  if AMaxWidth * LongDayFactor < ColWidth then
  ...
  begin
    ...
    if AMaxWidth * ShortDayFactor < ColWidth then
...
procedure TCustomPlannerViewEh.ResetResourcesViewArray;
var
  i: Integer;
begin
  if (PlannerDataSource <> nil) then
...
begin
  ...
  begin
    ...
    FStopGridRolPos := TTimeSpanDisplayItemEh(Source).StopGridRolPos;
...
begin
  ...
  RowCount := ARowCount;
  SetGridSize(FullColCount, 0, FullRowCount, 0);
  if ColGroups = 1
  ...
  begin
    ...
    begin
      ...
      MergeCells(0, ir, 1, FBarsPerHour);
      aTime := EncodeTime(i+AStartHour,0,0,0);
      CellValues[0,ir] := aTime;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      MergeCells(ir, FResourceAxisPos, FBarsPerRes-FInterResourceCols, 1);
   end;
  end;
...
begin
  ...
  begin
    VCellRect := CellRect(0, FAllDayRowIndex);
    Pos := Point(X, Y - VCellRect.Top);
    ...
    begin
      ...
      begin
        ...
        if ASpanRect.Top < 0 then
          ASpanRect.Top := 0;
        ASpanRect.Bottom := SpanItem.StopGridRolPos;
...
begin
  ...
  if FPlannerState = psSpanBottomSizingEh then
  begin
...
begin
  ...
  begin
    ...
    begin
      ic := FBarsPerRes * i;
      MergeCells(ic + FDataColsOffset, FResourceAxisPos, FBarsPerRes - 1, 1);
    end;
  end;
...
var
  ...
  begin
    Result := False;
    DecodeDate(FStartDate + AddDays, AYear, AMonth, ADay);
...
begin
  ...
  if CheckDrawMon then
    s := FormatDateTime('D MMM', FStartDate + AddDays)
  else
    s := FormatDateTime('D', FStartDate + AddDays);
...
begin
  ...
  FBandOrientation := atboVerticalEh;
  FTimeAxis := VertAxis;
...
procedure TPlannerAxisTimelineViewEh.SetRangeKind(const Value: TTimePlanRangeKind);
var
  AdjustedDate: TDateTime;
begin
  if FRangeKind <> Value then
  begin
    FRangeKind := Value;
...
procedure TPlannerAxisTimelineViewEh.SetBandOrientation(
  const Value: TAxisTimeBandOrientationEh);
begin
  if FBandOrientation <> Value then
  begin
    FBandOrientation := Value;
    if FBandOrientation = atboVerticalEh then
    begin
      FTimeAxis := VertAxis;
      FResourceAxis := HorzAxis;
    end else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if StackSpanItem = nil then
        begin
          if BandOrientation = atboVerticalEh then
          begin
            ASpanItem.FInCellCols := CurColBarCount;
            ASpanItem.FInCellFromCol := i;
            ASpanItem.FInCellToCol := i;
...
begin
  ...
  ASpanItem.FAllowedInteractiveChanges :=
    [sichSpanMovingEh, sichSpanTopSizingEh, sichSpanBottomSizingEh];
...
begin
  ...
  begin
    ...
    if FDayGroupCol >= 0 then
      MergeCells(FDayGroupCol, InGroupStartRow, 1, FDayGroupRows);

    if FHoursBarIndex >= 0 then
      for i := 0 to BarsInGroup - 1 do
      begin
        ir := InGroupStartRow + i*2;
        MergeCells(FHoursBarIndex, ir, 1, 2);
        CellValues[FHoursBarIndex, ir] := EncodeTime(i,0,0,0);
      end;
  end;
end;
...
begin
  case RangeKind of
    ...
    raise Exception.Create('TPlannerVertDaysLineViewEh.RangeKind is invalid');
  end;
end;
...
begin
  ...
  BandOrientation := atboHorizontalEh;
...
begin
  ...
  begin
    ...
    if FDayGroupRow >= 0 then
      MergeCells(InGroupStartCol, FDayGroupRow, FDayGroupCols, 1);

    if FHoursBarIndex >= 0 then
      for i := 0 to BarsInGroup - 1 do
      begin
        ir := InGroupStartCol + i*2;
        MergeCells(ir, FHoursBarIndex, 2, 1);
        CellValues[ir, FHoursBarIndex] := EncodeTime(i,0,0,0);
      end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ASpanRect.Right := SpanItem.StopGridRolPos;
...
procedure TMoveHintWindow.WMEraseBackground(var Message: TWmEraseBkgnd);
begin
  Message.Result := 1;
end;
...
begin
  case RangeKind of
    ...
    raise Exception.Create('TPlannerVertHoursLineViewEh.RangeKind is invalid');
  end
end;
...
function TPlannerDrawStyleEh.GetActualTodayFrameColor: TColor;
begin
...
begin
  ...
  if DrawArgs.HighlightToday and (DrawArgs.TodayDate = sbTrueEh) then
  begin
    Canvas.Brush.Color := ApproximateColor(Canvas.Brush.Color, GetActualTodayFrameColor, 128);
