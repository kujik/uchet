...
uses
 Messages, Contnrs, Variants, StrUtils,
  ...
  {$IFDEF FPC}
    EhLibLclUtils, LMessages, LCLType, MaskEdit, LCLIntf,
    {$IFDEF FPC_CROSSP}
    {$ELSE}
      ComCtrls, Windows,
    {$ENDIF}
  {$ELSE}
    EhLibVclUtils, DBConsts, Mask, RichEdit, ComCtrls, Windows,
  {$ENDIF}
  DBUtilsEh, SysUtils, Classes, Controls, Forms, Graphics, Menus,
  StdCtrls, ExtCtrls, Buttons, Db, DBCtrls, Imglist, GridsEh, EhLibUtils,
...
{ IInplaceEditHolderEh }
  ...
  IInplaceEditHolderEh = interface
    ...
    procedure GetMouseDownInfo(var Pos: TPoint; var Time: Integer);
...
{$ENDIF}
  ...
  public
    ...
    function IsDataIndependent: Boolean; virtual;
    procedure Modified;
    ...
    property DataIndependent: Boolean read FDataIndependent;
...
{ TCustomDBEditEh }
  ...
  protected
    ...
    function DataIndependent: Boolean; virtual;
    ...
    function GetDataSource: TComponent; virtual;
    ...
    procedure BeforeShowDefaultEditDropDownForm(EditControl: TControl; Button: TEditButtonEh; var DropDownForm: TCustomForm; DynParams: TDynVarsEh); virtual;
  ...
  public
    ...
    procedure SelectAll(CursortAtStart: Boolean); overload;
...
{ TCustomDBComboBoxEh }
  ...
  protected
    ...
    function TextListIndependent: Boolean;
...
begin
  if DataIndependent then Result := True
...
begin
  Result := ((Field <> nil) and Field.CanModify) or DataIndependent;
end;
...
begin
  if FDataIndependent <> IsDataIndependent then
  begin
    OldDataIndepended := FDataIndependent;
    FDataIndependent := IsDataIndependent;
    DataIndependentValue := Null;
    LayoutChanged;
    if not OldDataIndepended and FDataIndependent then
      RecordChanged(nil);
  end;
end;

function TFieldDataLinkEh.IsDataIndependent: Boolean;
begin
...
begin
  ...
  begin
    ...
    ADropDownFormParams.FOnOpenDropDownFormProc := DBEditControl.BeforeShowDefaultEditDropDownForm;
...
begin
  ...
  begin
    if (FDataLink <> nil) and (AComponent = FDataSource)
    then
      DataSource := nil
...
function TCustomDBEditEh.DataIndependent: Boolean;
begin
  Result := FDataLink.DataIndependent;
end;

function TCustomDBEditEh.GetDataSet: TDataSet;
begin
...
function TCustomDBEditEh.GetDBDataSource: TDataSource;
begin
  Result := DataLink.DataSource;
end;

function TCustomDBEditEh.GetDataSource: TComponent;
begin
  Result := FDataSource;
end;
...
procedure TCustomDBEditEh.SetDataSource(Value: TComponent);
begin
...
procedure TCustomDBEditEh.DrawEditImage();
var
  ImRect: TRect;
  ei: TEditImageEh;
begin
  ...
  begin
    ...
    DrawImage(Canvas, ImRect, ei.Images, ei.ImageIndex, False);
  end;
end;
...
begin
  if (csDesigning in ComponentState) and not DataIndependent then Exit;
  if not DataIndependent then DataSet.Edit;
    ...
    begin
      ...
      begin
        if FDataLink.Field.IsBlob then
          AValue := FDataLink.Field.AsString
        else if FDataLink.Field.EditMask <> '' then
          AValue := FDataLink.Field.AsString
        else
...
begin
  OldText := '';
  if (evEditMaskEh in FAssignedValues) and
     (Value = inherited EditMask)
  then
    Exit;
  ...
  if (csLoading in ComponentState) and (OldText <> '') and DataIndependent then
...
begin
  Result := (Text <> '') and DataIndependent;
end;

function TCustomDBEditEh.IsValueStored: Boolean;
begin
  Result := (Value <> Null) and DataIndependent;
end;
...
procedure TCustomDBEditEh.BeforeShowDefaultEditDropDownForm(EditControl: TControl;
...
procedure ResetEditButtonControl(WinControlEh: TWinControl; ControlRec: TEditButtonControlLineRec;
  Index: Integer; Flat: Boolean; MaxButtonHeight: Integer;
  var MinButtonHeight: Integer);
var
  AButtonHeight: Integer;
  DefaultFlatButtonWidth: Integer;
begin
  if WinControlEh is TEditButtonsBoxEh then
    DefaultFlatButtonWidth := TEditButtonsBoxEh(WinControlEh).DefaultFlatButtonWidth
  else if WinControlEh is TCustomControlEh then
    DefaultFlatButtonWidth := TCustomControlEh(WinControlEh).DefaultFlatButtonWidth
  else
    DefaultFlatButtonWidth := 0;
  ...
  if not ControlRec.EditButton.Visible then
    ...
    ControlRec.EditButtonControl.Width := DefaultFlatButtonWidth
  else
...
procedure TCustomDBEditEh.SelectAll(CursortAtStart: Boolean);
begin
  if CursortAtStart then
    SetSel(Length(Text), 0)
  else
    inherited SelectAll;
end;

procedure TCustomDBEditEh.SetSelection(SelStart, SelLength: Integer);
var
  SelStop: Integer;
begin
  if SelLength = MAXINT
    then SelStop := TextLength(Text)
    else SelStop := SelLength + SelStart;
...
{$IFDEF EH_LIB_29} 
        EmptyDataInfo.Font.ChangeScale(M, D, isDpiChange);
{$ELSE}
...
var
  ...
  MaskPos: TDateTimeElementsMaskPosEh;
begin
  DecodeDateTime(0, Y, M, D, H, N, S, MS);
  dts := DateTimeStamp;
  MaskPos := DateTimeMaskPos;
  if (dts.Year = -1) and
     (dts.Month = -1) and
     (dts.Day = -1) and
     (dts.Hour = -1) and
  ...
  begin
    if not MaskPos.Year.Present and (dts.Year <= 0) then
      dts.Year := 1;
    if not MaskPos.Month.Present and (dts.Month <= 0) then
      dts.Month := 1;
    if not MaskPos.Day.Present and (dts.Day <= 0) then
      dts.Day := 1;
    if not MaskPos.Hour.Present and (dts.Hour < 0) then
      dts.Hour := 0;
    if not MaskPos.Min.Present and (dts.Minute < 0) then
      dts.Minute := 0;
    if not MaskPos.Sec.Present and (dts.Second < 0) then
      dts.Second := 0;
  end;
  ...
  begin
    ...
    if MaskPos.Year.Present and (dts.Year > 0) then
      Y := dts.Year;
    if MaskPos.Month.Present and (dts.Month > 0) then
      M := dts.Month;
    if MaskPos.Day.Present and (dts.Day > 0) then
      D := dts.Day;
    if MaskPos.Hour.Present and (dts.Hour >= 0) then
      H := dts.Hour;
    if MaskPos.Min.Present and (dts.Minute >= 0) then
      N := dts.Minute;
    if MaskPos.Sec.Present and (dts.Second >= 0) then
      S := dts.Second;
    if AutoCorrect then
    begin
      CorrectDate(Y, M, D);
...
procedure TCustomDBDateTimeEditEh.CMCancelMode(var Message: TCMCancelMode);
  function CheckActiveListChildren: Boolean;
  var i: Integer;
  begin
    Result := False;
    if FDropDownCalendar <> nil then
...
begin
  ...
  if (Message.Sender <> Self) and (Message.Sender <> FDropDownCalendar) and
    not ContainsControl(Message.Sender) and not CheckActiveListChildren then
    CloseUp(False);
end;
{$ENDIF}
...
function TCustomDBComboBoxEh.TextListIndependent: Boolean;
begin
...
var
  ...
  PopupList: TComboBoxPopupListboxEh;
begin
  ...
  begin
    ...
    if Assigned(FOnDropDown) then FOnDropDown(Self);
    PopupList := TComboBoxPopupListboxEh(PopupListbox);
    PopupList.HandleNeeded;
    GetItemsList;
    PopupList.Color := GetPopupListboxColor;
    PopupList.Font := SelfPopupListboxFont;
    PopupList.ImageList := Self.Images;
    PopupList.ItemHeight := PopupList.GetTextHeight;
    if (Images <> nil) and
       (EditImage.UseImageHeight) and
       (PopupList.ItemHeight < Images.Height + 1)
    then
      PopupList.ItemHeight := Images.Height;
    PopupList.ClearFilter;
    PopupList.ExtItems := nil;
    PopupList.ExtItems := Self.Items;
    PopupList.Count := PopupList.ExtItems.Count;
    PopupList.RowCount := DropDownBox.Rows;
    if Self.ItemIndex < PopupList.Count then
      PopupList.ItemIndex := Self.ItemIndex;
    if (FDropDownBox.Width = -1) then
      PopupList.ClientWidth := GetItemsMaxWidth
    else if FDropDownBox.Width > 0 then
      PopupList.Width := FDropDownBox.Width
    else
      PopupList.Width := Self.Width;
    if (PopupList.Width < Self.Width) then PopupList.Width := Self.Width;
    if PopupList.Count < PopupList.RowCount then PopupList.RowCount := PopupList.Count;
    PopupList.SizeGripAlwaysShow := Self.DropDownBox.Sizable;
    ...
    PopupList.MasterFocusControl := Self;
...
begin
  ...
  begin
    ...
    if PopupList.SizeGripResized then
    begin
      DropDownBox.Rows := PopupList.RowCount;
      DropDownBox.Width := PopupList.Width;
    end;
    ...
    if Accept and not ReadOnly and FDataLink.Edit then
    begin
      InternalSetItemIndex(PopupList.ItemIndex);
      if FFocused then SelectAll;
    end;
...
begin
  ...
  if (Key = VK_DELETE) and
     ...
     ((Assigned(Field) and not Field.Required) or DataIndependent) and
     not ReadOnly and
     EditCanModify
  then
    InternalSetValue(Null);
...
begin
  Result := False;
  if DataIndependent or (FDataLink.Field <> nil) then
    if EditCanModify then
    begin
      ...
      begin
        ...
        begin
          OldSelLength := Abs(SelLength);
          SelStart := MAXINT;
          SelLength := -OldSelLength - 1;
        end;
        SetPopupListboxFilterFromSelection;
      end else
      begin
...
begin
  if DataIndependent and not FKeyBased
    then FDataLink.SetText(EditText)
...
begin
  ...
  FMinValue := AValue;
  if not (csLoading in ComponentState) and DataIndependent then UpdateData;
...
begin
  if (evMaxValueEh in FAssignedValues) and (AValue = FMaxValue)
    then Exit;
  FMaxValue := AValue;
  if not (csLoading in ComponentState) and DataIndependent
    then UpdateData;
...
begin
  ...
  if (Message.Sender = Self) or
    ((Message.Sender <> DropDownCalculator) and
      not ContainsControl(Message.Sender) and not CheckActiveListChildren)
  then
    CloseUp(False);
end;
{$ENDIF}
...
function TCustomDBCheckBoxEh.DataIndependent: Boolean;
begin
  Result := FDataLink.DataIndependent;
end;
...
function TCustomDBCheckBoxEh.GetDataSet: TDataSet;
begin
  if DBDataSource <> nil then
    Result := DBDataSource.DataSet
  else
...
function TCustomDBCheckBoxEh.GetDBDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

function TCustomDBCheckBoxEh.GetDataSource: TComponent;
begin
  Result := FDataSource;
end;
...
var
  Text: string;
begin
  if FDataLink.DataIndependent then
  begin
    Result := cbGrayed;
...
begin
  ...
  begin
    ...
    CheckRect := AdjustCheckBoxRect(Rect(0,0,Width,Height), taRightJustify, tlCenter,
                  FDefaultCheckBoxWidth, FDefaultCheckBoxHeight);
...
procedure TCustomDBCheckBoxEh.SetDataSource(Value: TComponent);
begin
  if Value = FDataSource then Exit;
...
begin
  if (csDesigning in ComponentState) and not FDataLink.DataIndependent then Exit;
  if not DataIndependent then DataSet.Edit;
...
begin
  Result := (DataIndependent and (State <> cbUnchecked));
end;
...
procedure TCustomDBCheckBoxEh.WMEraseBackground(var Message: TWmEraseBkgnd);
begin
...
procedure TCustomDBCheckBoxEh.GetCheckSize;
var
  b: TBitmap;
{$IFDEF FPC_CROSSP}
...
begin
  ...
  {$ELSE}
  {$HINTS OFF}
  StrLen := SendMessage(Memo.Handle, EM_GETLINE, Index, Integer(@Text));
  {$HINTS ON}
  {$ENDIF}
...
function TCustomDBImageEh.GetDataSet: TDataSet;
begin
  if DBDataSource <> nil then
    Result := DBDataSource.DataSet
  else
...
function TCustomDBImageEh.GetDBDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

function TCustomDBImageEh.GetDataSource: TComponent;
begin
  Result := FDataSource;
end;

procedure TCustomDBImageEh.SetDataSource(Value: TComponent);
begin
  if Value = FDataSource then Exit;
...
begin
  if not DataLink.DataIndependent then
  begin
    FDataUpdating := True;
    try
...
begin
  Result := DataLink.DataIndependent;
end;
...
begin
  ...
  ResetEditButtonControl(Self, EditButtonControlLineRec, 0, False, MaxButtonHeight, MinButtonHeight);
...
function TCustomDBRadioGroupEh.GetDataSet: TDataSet;
begin
  if DBDataSource <> nil then
    Result := DBDataSource.DataSet
  else
...
function TCustomDBRadioGroupEh.GetDBDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

function TCustomDBRadioGroupEh.GetDataSource: TComponent;
begin
  Result := FDataSource;
end;

procedure TCustomDBRadioGroupEh.SetDataSource(Value: TComponent);
begin
  if Value = FDataSource then Exit;
...
function TCustomDBRadioGroupEh.DataIndependent: Boolean;
begin
  Result := FDataLink.DataIndependent;
end;
...
begin
  ...
  Filer.DefineProperty('RtfText', ReadRtfText,
    WriteRtfText, HandleAllocated and (Lines.Count > 0) and DataIndependent);
end;
...
function TCustomDBRichEditEh.GetDataSet: TDataSet;
begin
  if DBDataSource <> nil then
    Result := DBDataSource.DataSet
  else
...
function TCustomDBRichEditEh.GetDBDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

function TCustomDBRichEditEh.GetDataSource: TComponent;
begin
  Result := FDataSource;
end;

procedure TCustomDBRichEditEh.SetDataSource(Value: TComponent);
begin
  if Value = FDataSource then Exit;
...
function TCustomDBRichEditEh.DataIndependent: Boolean;
begin
  Result := FDataLink.DataIndependent;
end;
...
var
  ...
  ButtonRec: TEditButtonControlLineRec;
begin
  MinButtonHeight := MAXINT;

  for i := 0 to Length(FBtnCtlList)-1 do
  begin
    ResetEditButtonControl(Self, FBtnCtlList[i], i, Flat, MaxButtonHeight, MinButtonHeight);
  ...
  begin
    for i := 0 to Length(FBtnCtlList)-1 do
    begin
      ButtonRec := FBtnCtlList[i];
      if inherited UseRightToLeftAlignment then
      begin
        ButtonRec.EditButtonControl.SetBounds(Indent - ButtonRec.EditButtonControl.Width, 0, ButtonRec.EditButtonControl.Width, MinButtonHeight);
        Dec(Indent, ButtonRec.EditButtonControl.Width);
        ButtonRec.ButtonLine.SetBounds(Indent - ButtonRec.ButtonLine.Width, 0, ButtonRec.ButtonLine.Width, MinButtonHeight);
        Dec(Indent, ButtonRec.ButtonLine.Width);
      end else
      begin
        ButtonRec.EditButtonControl.SetBounds(Indent, 0, ButtonRec.EditButtonControl.Width, MinButtonHeight);
        Inc(Indent, ButtonRec.EditButtonControl.Width);
        ButtonRec.ButtonLine.SetBounds(Indent, 0, ButtonRec.ButtonLine.Width, MinButtonHeight);
        Inc(Indent, ButtonRec.ButtonLine.Width);
      end;
    end;
  end;
...
procedure TEditButtonsBoxEh.WMEraseBackground(var Message: TWmEraseBkgnd);
begin
  inherited;
end;
...
procedure TEditButtonsBoxEh.ChangeScale(M, D: Integer {$IFDEF EH_LIB_24}; isDpiChange: Boolean {$ENDIF});
begin
  inherited ChangeScale(M, D {$IFDEF EH_LIB_24}, isDpiChange{$ENDIF});
  FDefaultFlatButtonWidth := GetDefaultFlatButtonWidth();
end;

function TEditButtonsBoxEh.GetDefaultFlatButtonWidth: Integer;
{$IFDEF FPC_CROSSP}
var
