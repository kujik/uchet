...
{                  TCustomMemTableEh                    }
{                                                       }
{    Copyright (c) 2003-2025 by Dmitry V. Bolshakov     }
{                                                       }
{*******************************************************}
...
{ sort data in this DataSet using                       }
...
{$ELSE}
  ...
  {$ENDIF}
{$ENDIF}
  EhLibUtils, DBUtilsEh, DBGridEh.DBUtils,
...
type
  ...
  TMTEDatasetFeaturesEh = class(TDBGridSQLDatasetFeaturesEh)
  protected
...
begin
  Result := False;
  MemTable := (AGrid.DataSet as TCustomMemTableEh);
...
begin
  ...
  begin
    ...
    if DS <> nil then
    begin
      DatasetFeatures := GetDBGridDatasetFeaturesForDataSet(DS);
...
begin
  mt := nil;
  Result := False;
  if not ( (AGrid.DataSet <> nil) and AGrid.DataSet.Active ) then
    Exit;
  if (AGrid.DataSet is TCustomMemTableEh) then
    mt := (AGrid.DataSet as TCustomMemTableEh);
...
begin
  if Sender is TCustomDBGridEh then
    ...
    Exit;
  if not ( (Grid.DataSet <> nil) and Grid.DataSet.Active ) then
    Exit;
  FBaseNode := nil;
  if (Grid.DataSet is TCustomMemTableEh) then
  begin
    mt := (Grid.DataSet as TCustomMemTableEh);
...
initialization
  RegisterDBGridDatasetFeaturesEh(TMTEDatasetFeaturesEh, TCustomMemTableEh);
