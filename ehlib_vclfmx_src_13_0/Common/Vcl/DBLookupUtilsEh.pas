...
uses
  ...
  {$ELSE}
    EhLibVclUtils,Messages,
  {$ENDIF}
  EhLibUtils, DBUtilsEh, DBCtrlsEh;
...
begin
  if FLookupLink.Active and FLookupLink.DataSet.IsLinkedTo(TAxisBarEh(FAxisBar).Grid.DBDataSource) then
  {$IFDEF FPC}
...
begin
  Result := TAxisBarEhCrack(FAxisBar).Grid.DBDataSource;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      try
        Bookmark := LookupDataSet.Bookmark;
        LookupDataSet.First;
        ...
        if DataSetBookmarkValid(LookupDataSet, Bookmark) then
          LookupDataSet.Bookmark := Bookmark;
      finally
