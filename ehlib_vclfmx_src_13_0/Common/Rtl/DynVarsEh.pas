...
{                                                       }
{                     EhLib 12.1                        }
{            Implementation Collection of               }
{                 Dynamic Variables                     }
{                                                       }
{     Copyright (c) 2012-2025 by EhLib Team and         }
{                Dmitry V. Bolshakov                    }
{                                                       }
{*******************************************************}
...
function TCustomDynVarEh.GetAsInteger: Integer;
begin
  if IsNull
    then Result := 0
    else Result := FData;
...
procedure TCustomDynVarEh.SetAsInteger(Value: Integer);
begin
  if IsDummy then
    FOwnerWhenDummy.CreateDynVarFromDummy(Self);
  Self.Value := Value;
...
begin
  ...
  if (ADinVar <> nil) and (ADinVar <> Self) then
    raise Exception.CreateFmt(EhLibLanguageConsts.DuplicateVarNameEh, [Value]);
  if IsDummy then
    FOwnerWhenDummy.CreateDynVarFromDummy(Self);
  FName := Value;
  Changed(False);
...
begin
  ...
  if Assigned(AVar) then
    raise Exception.CreateFmt('A Variable named ''%s'' does not exist in the Collection ''%s''', [VarName, TStringVarsEh.ClassName])
  else
    Result := AVar.Value;
end;
