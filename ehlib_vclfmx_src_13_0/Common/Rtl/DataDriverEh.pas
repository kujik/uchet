...
uses SysUtils, Classes, DB, Contnrs,
  Variants,
  DynVarsEh, EhLibUtils, DBUtilsEh, MemTableDataEh;
...
type
  ...
  TUpdateErrorActionEh = (ueaBreakAbortEh, ueaBreakRaiseEh, ueaContinueEh, ueaRetryEh, ueaContinueSkip);
...
{ IConnectionProviderDesignServiceEh }
...
begin
  ...
  begin
    ...
    if Action = ueaBreakAbortEh then
      Break;
    if Action <> ueaContinueSkip then
      MemRec.MergeChanges;
  end;
...
begin
  ...
  begin
    ...
    {$IFDEF FPC}
      FServerSpecOperationsClass := {%H-}TServerSpecOperationsEhClass(ServerTypeClasses.Objects[NewSTIndex]);
    {$ELSE}
