...
uses
  SysUtils, Variants, DBUtilsEh, ADODB, DB, DBGridEh, ToolCtrlsEh, MemTableEh,
  MemTableDataEh, DBGridEh.DBUtils;

type

  TADOSQLDatasetFeaturesEh = class(TDBGridSQLDatasetFeaturesEh)
  public
  ...
  TADOCommandTextDatasetFeaturesEh = class(TDBGridCommandTextDatasetFeaturesEh)
  public
    function GetLocalFilterApplyingWay: TLocalFilterApplyingWayEh; override;
    function NullComparisonSyntax(AGrid: TCustomDBGridEh; DataSet: TDataSet; IsLocalFilter: Boolean): TNullComparisonSyntaxEh; override;
    procedure ApplySorting(Sender: TObject; DataSet: TDataSet; IsReopen: Boolean); override;
...
var
  ...
  var
    i: Integer;
    RecsetFieldName: String;
  begin
    Result := -1;
    for i := 0 to Recordset.Fields.Count-1 do
    begin
      RecsetFieldName := Recordset.Fields.Item[i].Name;
      if AnsiCompareText(AFieldName, RecsetFieldName) = 0 then
//      if AFieldName = RecsetFieldName then
      begin
        Result := i;
        Exit;
      end;
...
initialization
  RegisterDBGridDatasetFeaturesEh(TADOSQLDatasetFeaturesEh, TADOQuery);
  RegisterDBGridDatasetFeaturesEh(TADOCommandTextDatasetFeaturesEh, TCustomADODataSet);
end.
