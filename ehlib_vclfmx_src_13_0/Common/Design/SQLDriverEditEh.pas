...
{ TCustomDBService }
  ...
  protected
    ...
    procedure AssignRecord(Source, Destination: TDataSet); virtual;
    procedure OnPopupClick(Sender: TObject);
...
{ TInterbaseDBService }
  ...
  protected
    procedure AssignRecord(Source, Destination: TDataSet); override;
  public
...
procedure TDesignDataBaseEh.mtDBTreeExpanding(Sender: TObject;
  ...
  function AddChildren(Node: TObject; RefParent: Variant): Integer;
  var
    List: TObjectList;
    i: Integer;
    sn: TSQLTreeNode;
...
begin
  ...
  begin
    ChildCount := AddChildren(VariantToRefObject(FTreeDataSet['RefData']), Id);
...
begin
  ...
  begin
    ChildCount := AddChildren(VariantToRefObject(vDBTree['RefData']), Id);
...
begin
  FTemplateList.AddObject(Template.FName, Template);
end;
...
begin
  ...
  if UpperCase(TemplateName) = UpperCase('_RecordAsList') then
    Result := nil
  else if FTemplateList.IndexOf(TemplateName) > -1 then
    Result := TSQLTreeNodeTemplate(FTemplateList.Objects
      [FTemplateList.IndexOf(TemplateName)]);
end;
...
begin
  ...
  if FTemplateList.Count = 0 then Exit;
  Template := TSQLTreeNodeTemplate(FTemplateList.Objects[0]);
...
procedure TCustomDBService.AssignRecord(Source, Destination: TDataSet);
begin
  MemTableEh.AssignRecord(Source, Destination);
end;
...
procedure TInterbaseDBService.AssignRecord(Source, Destination: TDataSet);
var
  i: Integer;
  Field: TField;
begin
    ...
    begin
      ...
      if Field <> nil then
        if Field is TStringField and not Field.IsNull then
          Destination.Fields[i].Value := TrimRight(VarToStr(Field.Value))
        else
...
procedure TSQLDataEditWin.gridDBTreeColumns0GetCellParams(Sender: TObject;
  EditMode: Boolean; Params: TColCellParamsEh);
begin
  Params.ImageIndex := TDBGridColumnEh(Sender).Grid.DBDataSource.DataSet.FieldByName('ImageIndex').AsInteger;
end;

procedure TSQLDataEditWin.gridTreeDetailColumns0GetCellParams(
  Sender: TObject; EditMode: Boolean; Params: TColCellParamsEh);
begin
  Params.ImageIndex := TDBGridColumnEh(Sender).Grid.DBDataSource.DataSet.FieldByName('Image_Index').AsInteger;
end;
