...
{$IFDEF EH_LIB_16} Themes, {$ENDIF}
  ...
  DataDriverEh, ADODataDriverEh, EhLibVclUtils, ADODB,
...
uses FrameOne, FrameTwo, FrameThree, FrameFour, FrameFive;
...
begin
  ...
  MemTableEh1.FieldByName('MenuName').AsString := '  Using DropDownForms.'#13' Method 1.';
  MemTableEh1.Post;

  MemTableEh1.Next;
  MemTableEh1.Edit;
  TRefObjectField(MemTableEh1.FieldByName('RefFrameClass')).Value := TObject(TfrFrameTwo);
  MemTableEh1.FieldByName('MenuName').AsString := '  Using DropDownForms.'#13' Method 2.';
  MemTableEh1.Post;

  MemTableEh1.Append;
  MemTableEh1.Edit;
  TRefObjectField(MemTableEh1.FieldByName('RefFrameClass')).Value := TObject(TfrFrameThree);
  MemTableEh1.FieldByName('MenuName').AsString := '  Using DropDownForms.'#13' Method 3.';
  MemTableEh1.Post;

  MemTableEh1.Append;
  MemTableEh1.Edit;
  TRefObjectField(MemTableEh1.FieldByName('RefFrameClass')).Value := TObject(TfrFrameFour);
  MemTableEh1.FieldByName('MenuName').AsString := '  Advanced way of using DropDownForm.';
  MemTableEh1.Post;
...
//    Frame.Parent := nil;
  ...
  begin
    ...
    Frame.Parent := Panel1;
    if (OldFrame <> nil) and (OldFrame <> Frame) then
      OldFrame.Parent := nil;
