...
procedure TfrMailBox.CreateWnd;
begin
  inherited CreateWnd;
...
begin
  ...
  if TForm1.DelphiPlatformInfo = 'WIN64' then
...
begin
  ...
  if (CustomStyleActive) then
  begin
    DBGridEh1.GridLineParams.DarkColor := clDefault;
    DBGridEh1.FixedColor := clBtnFace;
    DBGridEh1.TitleParams.VertLineColor := clDefault;
    //DBGridEh1.DataGrouping.Font.Color := clWindowText;
    DBGridEh1.DataGrouping.GroupRowDefValues.BottomLine.Color := clDefault;
  ...
  end else if DBComboBoxEh1.Value = 'Silver' then
  begin
