...
{$IFDEF EH_LIB_16} Themes, {$ENDIF}
  ...
  IniFiles, DataDriverEh, ADODataDriverEh, EhLibVclUtils, EhLibUtils,
  DBVertGridsEh, PivotGridsEh, PlannersEh,
  FrameMainGrid, FrameMasterDetail, FrameFishFact, FrameMailBox,
  EhLibVclMTE, DBGridEhGrouping, ToolCtrlsEh, DBGridEhToolCtrls,
...
type
  ...
  private
    ...
    procedure GetHintInfo(Sender: TLaObjectEh; EventArgs: TLaHintInfoEventArgs);
    procedure DBGridEh1LaControlLayoutNeeded(Grid: TCustomDBGridEh; Params: TDBGridLaControlLayoutNeededEventParamsEh);
...
{$ENDIF}
    ...
    class function DelphiPlatformInfo: String;
  end;
...
class function TForm1.DelphiPlatformInfo(): String;
begin
...
begin
  ...
  if DelphiPlatformInfo = 'WIN64' then
    ProviderStr := ProviderStr + 'Provider=Microsoft.ACE.OLEDB.12.0;'
  else
    ProviderStr := ProviderStr + 'Provider=Microsoft.Jet.OLEDB.4.0;';

  ADOConnectionProviderEh1.InlineConnection.ConnectionString :=
    ProviderStr + 'User ID=Admin;'+
...
begin
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'Edit Controls includes DBLookupComboboxEh, DBDateTimeEditEh, DBNumberEditEh, DBEditEh, DBComboBoxEh controls.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'Additional edit controls includes DBMemoEh, DBImageEh, DBRichEditEh.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'A vertical grid displays the data of a single record vertically in a column.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'A cell or picture in a grid can be displayed as a RefLink, i.e. display a special cursor pointer, highlight on hover.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'One or more buttons of various types can be embedded in a grid cell.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'By clicking on the Edit Button, special drop-down forms can be displayed.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'Each row in DBGrid can have a drop-down panel to display detailed information.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'Cells within a record can be arranged either one behind the other or one below the other. This mode is called RowPanel.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'Records in a grid can be arranged in the form of a tree. The Grid must be connected to a MemTablEh.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'In the grid, you can set an image as a background.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'In the grid, you can search and filter content through a special - SearchPanel.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'This demonstrates the capabilities of the grid as a display of a list of letters from email.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'Grid content can be exported to formats such as Text, HTML, RTF, Xls(Via OLE), Xslx directly, Pdf.';
  ...
  TRefObjectField(MemTableEh1.FieldByName('RefFrameClass')).Value := TObject(TfrOneFishFacts);
  MemTableEh1.Post;

  //MemTableEh - DataSet
  MemTableEh1.Append;
  MemTableEh1.FieldValues['MenuName'] := '' + ApplicationLanguageConsts.VertMenuMemTableEh;
  MemTableEh1.FieldValues['DetailInfo'] := 'A DataSet component for storing data in memory.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'Components for displaying events and tasks on the timeline.';
  ...
  MemTableEh1.FieldValues['DetailInfo'] := 'Components for import/export of text data to any DataSet';
  ...
  DBGridEh1.Columns[0].OnLaControlLayoutNeeded := DBGridEh1LaControlLayoutNeeded;
end;
...
function TForm1.CreateGridLaControls: TLaObjectEh;
begin
  ...
  begin
    ...
    begin
      Margins.SetBounds(14, 4, 4, 4);
      FieldName := 'MenuName';
    end;

    with TLaTextBlockEh.CreateWith(RefSelf) do
    begin
      Margins.SetBounds(28, 2, 2, 2);
      Font.Size := 7;
      Font.Color := ApproximateColor(CheckSysColor(clGrayText), CheckSysColor(DBGridEh1.Color), 100);
...
procedure TForm1.DBGridEh1LaControlLayoutNeeded(Grid: TCustomDBGridEh;
  Params: TDBGridLaControlLayoutNeededEventParamsEh);
var
  StackPanel: TLaStackPanelEh;
  TextBlock: TLaTextBlockEh;
begin
  StackPanel := Params.LaControl as TLaStackPanelEh;
  TextBlock := StackPanel.Children[0] as TLaTextBlockEh;
  if (gdCurrent in Params.CellState) then
    TextBlock.Font.Style := [TFontStyle.fsBold]
  else
    TextBlock.Font.Style := [];
end;

procedure TForm1.GetHintInfo(Sender: TLaObjectEh;
  EventArgs: TLaHintInfoEventArgs);
begin
  EventArgs.HintText := MemTableEh1.FieldByName('DetailInfo').AsString;
  EventArgs.Handled := True;
  EventArgs.HideTimeout := 8000; //8 sec
end;
...
initialization
  ...
  DBGridEhCenter.BuildMenuOnPopup := True;
  DBGridEhCenter.UseGlobalPopupMenu := True;
