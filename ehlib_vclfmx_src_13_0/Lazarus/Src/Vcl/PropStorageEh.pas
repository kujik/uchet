...
{$ELSE}
  ...
  {$ELSE}
  EhLibVclUtils, Mask, ComCtrls, Windows,
  {$ENDIF}
{$ENDIF}
  Classes, IniFiles, TypInfo, Contnrs, EhLibUtils;
...
function RegistryKeyToIdent(RootKey: Integer; var Ident: string): Boolean;
function IdentToRegistryKey(const Ident: string; var RootKey: Integer): Boolean;
...
begin
  ...
  begin
    if Token = '' then
      raise Exception.Create('Invalid property path: "' + APath + '"');
