...
{$IFDEF EH_LIB_17} System.UITypes, {$ENDIF}
  {$IFDEF FPC}
    ...
    {$ENDIF}
    EhLibLclUtils, LMessages, LCLType, MaskEdit, LCLIntf, LCLProc, LazUTF8,
  {$ELSE}
    EhLibVclUtils, Mask, Windows, UxTheme,
  {$ENDIF}
  Messages, Graphics, Menus, Controls, Forms, StdCtrls, Imglist,
  Types, Themes, GridToolCtrlsEh, ToolCtrlsEh, EhLibUtils,
...
{ TInplaceEdit }
  ...
  protected
    ...
    function GetDownBorderCellColor(ACol, ARow: Integer): TColor; virtual;
    ...
    function GetLeftBorderCellColor(ACol, ARow: Integer): TColor; virtual;
    function GetRightBorderCellColor(ACol, ARow: Integer): TColor; virtual;
    function GetTopBorderCellColor(ACol, ARow: Integer): TColor; virtual;
...
{ TGridAxisDataEh }
  ...
  protected
    ...
    procedure InsertRolCells(const Pos, Count: Integer);
    procedure DeleteRolCells(const Pos, Count: Integer);
  ...
  public
    ...
    property RolCelLens[Index: Integer]: Integer read GetRolCelLens write SetRolCelLens;
    ...
    property RolLocCelPosArr[Index: Integer]: Int64 read GetRolLocCelPosArr write SetRolLocCelPosArr;
...
{ TCustomGridEh }
  ...
  TSelectCellEvent = procedure (Sender: TObject; ACol, ARow: Integer;
...
{$ENDIF}
    ...
    procedure MoveCurrent(ACol, ARow: Integer; ShowX, ShowY: Boolean);
...
{$ENDIF}
  ...
  protected
    ...
    FMoveFromIndex, FMoveToIndex, FMoveFromCellOriginDistance: Integer;
...
{$ENDIF}
    ...
    function CellEditRect(ACol, ARow: Integer): TRect; virtual;
    function CellRect(ACol, ARow: Integer; IncludeLine: Boolean = False; UseRTL: Boolean = True): TRect;
    function CellRectAbs(ACol, ARow: Integer; IncludeLine: Boolean = False): TRect;
    ...
    function GetEditMask(ACol, ARow: Integer): string; virtual;
    function GetEditStyle(ACol, ARow: Integer): TEditStyle; virtual;
    ...
    procedure CalcSizingState(X, Y: Integer; var State: TGridStateEh; var Index: Integer; var SizingPos, SizingOfs: Integer); virtual;
    ...
    procedure DrawText(ACanvas: TCanvas; ARect: TRect; FillRect: Boolean; DX, DY: Integer; const Text: string; Alignment: TAlignment; Layout: TTextLayout; MultiL: Boolean; EndEllipsis: Boolean; LeftMarg, RightMarg: Integer; ForceSingleLine: Boolean; UseRightToLeftAlignment: Boolean);
    ...
    procedure FocusCell(ACol, ARow: Integer; MoveAnchor: Boolean); virtual;
    ...
    procedure MoveColRow(ACol, ARow: Integer; ShowX, ShowY: Boolean); virtual;
    ...
    procedure SetEditText(ACol, ARow: Integer; const Value: string); virtual;
    ...
    procedure WriteCellText(ACanvas: TCanvas; ARect: TRect; FillRect: Boolean; DX, DY: Integer; const Text: string; Alignment: TAlignment; Layout: TTextLayout; MultiL, EndEllipsis: Boolean; LeftMarg, RightMarg: Integer; ForceSingleLine: Boolean);
    ...
    property Col: Integer read FCurCell.X write SetCol;
    ...
    property Row: Integer read FCurCell.Y write SetRow;
    ...
    property SizeGripPosition: TSizeGripPosition read FSizeGripPosition write SetSizeGripPosition default sgpBottomRight;
...
{ TCustomListboxEh }
  ...
  protected
    ...
    property VisibleItems: TStrings read GetVisibleItems;
...
procedure ArrayMove(var Extents: TIntegerDynArray; FromIndex, ToIndex: Integer);
var
  Extent, I: Integer;
begin
...
procedure ArrayInsertRange(var Extents: TIntegerDynArray; StartIndex, Amount: Integer);
var
  I: Integer;
begin
...
procedure ArrayDeleteRange(var Extents: TIntegerDynArray; StartIndex, Amount: Integer);
var
  I: Integer;
begin
...
begin
  ...
  else if (SendCharKeyStr <> FCharKeyStr) and
          (Length(FCharKeyStr) = 1) then
  begin
    Key := FCharKeyStr[1];
  end;
...
{$ENDIF}
  ...
  FSizeGripPosition := sgpBottomRight;
...
function TCustomGridEh.CellRect(ACol, ARow: Integer;
  IncludeLine: Boolean = False; UseRTL: Boolean = True): TRect;
begin
...
function TCustomGridEh.CellEditRect(ACol, ARow: Integer): TRect;
begin
...
function TCustomGridEh.GetEditMask(ACol, ARow: Integer): string;
begin
  Result := '';
end;

function TCustomGridEh.GetEditText(ACol, ARow: Integer): string;
begin
  Result := '';
end;

procedure TCustomGridEh.SetEditText(ACol, ARow: Integer; const Value: string);
begin
end;
...
function TCustomGridEh.GetEditStyle(ACol, ARow: Integer): TEditStyle;
begin
  Result := esSimple;
end;
...
procedure TCustomGridEh.MoveColRow(ACol, ARow: Integer; ShowX, ShowY: Boolean);
begin
...
begin
  ...
  begin
    ...
    begin
      VCellRect.Left := HorzAxis.RolInClientBoundary;
      VCellRect.Right := HorzAxis.ContraStart;
      VCellRect.Bottom := VertAxis.GridClientStart;

      for RowI := 0 to FixedRowCount - 1 do
      begin
        VCellRect.Top := VCellRect.Bottom;
        VCellRect.Bottom := VCellRect.Top + RowHeights[RowI];
        DrawEmptyAreaCell(-1, RowI, VCellRect);
      end;
    end;

    VCellRect.Left := HorzAxis.RolInClientBoundary;
    VCellRect.Bottom := VertAxis.FixedBoundary - VertAxis.RolStartVisCelOfst;
    VCellRect.Right := HorzAxis.ContraStart;
    ...
    begin
      VCellRect.Top := VCellRect.Bottom;
      VCellRect.Bottom := VCellRect.Top + RowHeights[RowI];
      if VCellRect.Top > VertAxis.ContraStart then
        Break;
      DrawEmptyAreaCell(-1, RowI, VCellRect);
    end;

    VCellRect.Left := HorzAxis.RolInClientBoundary;
    VCellRect.Top := VertAxis.RolInClientBoundary;
    VCellRect.Right := HorzAxis.ContraStart;
    VCellRect.Bottom := VertAxis.ContraStart;
    DrawEmptyAreaCell(-1, -1, VCellRect);
  ...
  begin
    ...
    begin
      VCellRect.Top := VertAxis.RolInClientBoundary;
      VCellRect.Right := HorzAxis.GridClientStart;
      VCellRect.Bottom := VertAxis.ContraStart;

      for ColI := 0 to FixedColCount - 1 do
      begin
        VCellRect.Left := VCellRect.Right;
        VCellRect.Right := VCellRect.Left + ColWidths[ColI];
        DrawEmptyAreaCell(ColI, -1, VCellRect);
      end;
    end;
    ...
    RestRgn := SelectClipRectangleEh(Canvas, RestRect);
    VCellRect.Top := VertAxis.RolInClientBoundary;
    VCellRect.Right := HorzAxis.FixedBoundary - HorzAxis.RolStartVisCelOfst;
    VCellRect.Bottom := VertAxis.ContraStart;
    ...
    begin
      VCellRect.Left := VCellRect.Right;
      VCellRect.Right := VCellRect.Left + ColWidths[ColI];
      if VCellRect.Left > HorzAxis.ContraStart then
        Break;
      DrawEmptyAreaCell(ColI, -1, VCellRect);
    end;
    RestoreClipRectangleEh(Canvas, RestRgn);
...
function TCustomGridEh.CheckCellLine(ACol, ARow: Integer; BorderType: TGridCellBorderTypeEh): Boolean;
var
  IsDraw: Boolean;
  BorderColor: TColor;
  IsExtent: Boolean;
...
procedure TCustomGridEh.CalcSizingState(X, Y: Integer;
  var State: TGridStateEh; var Index: Integer; var SizingPos, SizingOfs: Integer);
...
procedure TCustomGridEh.DrawText(ACanvas: TCanvas; ARect: TRect;
  ...
  Layout: TTextLayout; MultiL: Boolean;
...
begin
  ...
  if FDrawnSizingPos1 >= 0 then
  begin
    if FGridState = gsColSizingEh then
    begin
      Canvas.MoveTo(FDrawnSizingPos1-1, StartPos);
      Canvas.LineTo(FDrawnSizingPos1-1, FinishPos);
    end else
    begin
      Canvas.MoveTo(StartPos, FDrawnSizingPos1-1);
      Canvas.LineTo(FinishPos, FDrawnSizingPos1-1);
    end;
  end;
end;
...
begin
  ...
  if FGridState = gsColSizingEh then
  begin
    InvalidRect := Rect(FDrawnSizingPos2-1, 0, FDrawnSizingPos2, ClientHeight);
  ...
  begin
    InvalidRect := Rect(0, FDrawnSizingPos2-1, ClientWidth, FDrawnSizingPos2);
    WindowsInvalidateRect(Handle, InvalidRect, False);
...
begin
  ...
  begin
    ...
    begin
      ...
      FDrawnSizingPos1 := HorzAxis.FixedBoundary + HorzAxis.RolLocCelPosArr[FSizingIndex-FixedColCount] - HorzAxis.RolStartVisPos
    else
      FDrawnSizingPos1 := -1;
  ...
  begin
    if FSizingIndex - FixedRowCount >= 0 then
      FDrawnSizingPos1 := VertAxis.FixedBoundary + VertAxis.RolLocCelPosArr[FSizingIndex-FixedRowCount] - VertAxis.RolStartVisPos
    else
      FDrawnSizingPos1 := -1;
...
procedure TCustomGridEh.FocusCell(ACol, ARow: Integer; MoveAnchor: Boolean);
begin
...
procedure TCustomGridEh.InvalidateCell(ACol, ARow: Integer);
var
  Rect: TGridRect;
begin
  Rect.Top := ARow;
...
procedure TCustomGridEh.InvalidateCol(ACol: Integer);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
procedure TCustomGridEh.InvalidateRow(ARow: Integer);
var
  Rect: TGridRect;
begin
  if not HandleAllocated then Exit;
...
const
  ...
  HorzScrollFlags: array[Boolean] of Integer = (SB_LINERIGHT, SB_LINELEFT);
var
        ...
        begin
          ...
          begin
            for I := 0 to LCols - 1 do
              ScrollBarMessage(SB_HORZ, HorzScrollFlags[DeltaX > 0], 0, True);
            FPanPoint := EventInfo.Location;
...
begin
  ...
  if (FHotTrackCell.X >= 0) and (FHotTrackCell.Y >= 0) then
  begin
    VCellRect := CellRect(FHotTrackCell.X, FHotTrackCell.Y);
    FHotTrackInCellPos.X := X - VCellRect.Left;
    FHotTrackInCellPos.Y := Y - VCellRect.Top;
  end;
...
begin
  ...
  begin
    ...
    begin
      VCellRect := CellRect(CellHit.X, CellHit.Y);
      HTSpot.InCellX := X - VCellRect.Left;
      ...
      HTSpot.InCellY := Y - VCellRect.Top;
...
begin
  ...
  begin
    ...
    begin
      ...
      VCellRect := CellRect(CellHit.X, CellHit.Y);
      FMoveFromCellOriginDistance := X - VCellRect.Left;
...
{$ENDIF}
  ...
  begin
    ...
    InCellMousePos := Point(MouseX - VCellRect.Left, MouseY - VCellRect.Top);
...
begin
  ...
  if (FGridState = gsColMovingEh) and
     ...
     (CellHit = FMoveFromIndex + 1 + SkipHiddenCells(FMoveFromIndex + 1)) and
...
procedure TCustomGridEh.SetCol(const Value: Integer);
begin
  if Col <> Value then
    FocusCell(Value, Row, True);
end;

procedure TCustomGridEh.SetRow(const Value: Integer);
begin
  if Row <> Value then
...
procedure TCustomGridEh.SetLeftCol(Value: Integer);
begin
...
procedure TCustomGridEh.SetTopRow(Value: Integer);
begin
...
procedure TCustomGridEh.WMEraseBackground(var Message: TWMEraseBkgnd);
begin
  Message.Result := 1;
end;
...
procedure TCustomGridEh.DeleteColumn(ACol: Integer);
begin
  MoveColumn(ACol, FullColCount-1);
  ColCount := ColCount - 1;
end;

procedure TCustomGridEh.DeleteRow(ARow: Integer);
begin
  LockPaint;
  try
    VertAxis.DeleteRolCells(ARow-VertAxis.FixedCelCount, 1);
  finally
    UnlockPaint;
  end;
end;
...
begin
  ...
  begin
    ...
    ASizeGripPosition := sgpBottomRight;
    if UseRightToLeftAlignment then
      ASizeGripPosition := ChangeRightToLeftSizeGripPosition(sgpBottomRight);
  ...
  begin
    ...
    end else if SizeGripPosition = sgpTopRight then
    begin
    ...
    if UseRightToLeftAlignment then
      ASizeGripPosition := ChangeRightToLeftSizeGripPosition(ASizeGripPosition);
...
function TCustomDrawGridEh.CellRect(ACol, ARow: Integer): TRect;
begin
  Result := inherited CellRect(ACol, ARow);
end;

procedure TCustomDrawGridEh.MouseToCell(X, Y: Integer; var ACol, ARow: Integer);
var
  Coord: TGridCoord;
begin
...
procedure TCustomDrawGridEh.ColumnMoved(FromIndex, ToIndex: Integer);
begin
...
function TCustomDrawGridEh.GetEditMask(ACol, ARow: Integer): string;
begin
  Result := '';
  if Assigned(FOnGetEditMask) then FOnGetEditMask(Self, ACol, ARow, Result);
end;

function TCustomDrawGridEh.GetEditText(ACol, ARow: Integer): string;
begin
  Result := '';
  if Assigned(FOnGetEditText) then FOnGetEditText(Self, ACol, ARow, Result);
end;

procedure TCustomDrawGridEh.RowMoved(FromIndex, ToIndex: Integer);
begin
  if Assigned(FOnRowMoved) then FOnRowMoved(Self, FromIndex, ToIndex);
end;

function TCustomDrawGridEh.SelectCell(ACol, ARow: Integer): Boolean;
begin
  Result := True;
  if Assigned(FOnSelectCell) then FOnSelectCell(Self, ACol, ARow, Result);
end;

procedure TCustomDrawGridEh.SetEditText(ACol, ARow: Integer; const Value: string);
begin
  if Assigned(FOnSetEditText) then FOnSetEditText(Self, ACol, ARow, Value);
end;

procedure TCustomDrawGridEh.DrawCell(ACol, ARow: Integer; ARect: TRect;
...
begin
  ...
  begin
    ...
    if Delta > 0 then
      InsertRolCells(FRolCelCount, Delta)
    else if Delta < 0 then
      DeleteRolCells(FRolCelCount+Delta, -Delta);
  end;
end;

procedure TGridAxisDataEh.InsertRolCells(const Pos, Count: Integer);
begin
...
procedure TGridAxisDataEh.DeleteRolCells(const Pos, Count: Integer);
begin
...
begin
  if (Value >= 0) and ((Row <> Value) or (FRowSelected = False)) then
  begin
    Row := Value;
    FRowSelected := True;
    InvalidateIndex(Value);
  end else if (FRowSelected = True) and (Value = -1) then
  begin
    Row := 0;
    FRowSelected := False;
    InvalidateIndex(Value);
...
function TCustomListboxEh.GetVisibleItems: TStrings;
begin
...
procedure TCustomListboxEh.ResetVisibleItems;
var
  BaseList: TStrings;
  i: Integer;
  IntAsObj: TObject;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        FVisibleItems.AddObject(BaseList[i], IntAsObj);
      end;
    end;
  finally
    FVisibleItems.EndUpdate;
  end;
end;
...
begin
  ...
  begin
    if SizeGripPosition = sgpTopRight then SizeGripPosition := sgpTopLeft
    else if SizeGripPosition = sgpBottomRight then SizeGripPosition := sgpBottomLeft;
  end else if NewPosition = sgcpToRight then
  begin
    if SizeGripPosition = sgpTopLeft then SizeGripPosition := sgpTopRight
    else if SizeGripPosition = sgpBottomLeft then SizeGripPosition := sgpBottomRight
  end else if NewPosition = sgcpToTop then
  begin
    if SizeGripPosition = sgpBottomRight then SizeGripPosition := sgpTopRight
    else if SizeGripPosition = sgpBottomLeft then SizeGripPosition := sgpTopLeft
  end else if NewPosition = sgcpToBottom then
  begin
    if SizeGripPosition = sgpTopRight then SizeGripPosition := sgpBottomRight
    else if SizeGripPosition = sgpTopLeft then SizeGripPosition := sgpBottomLeft
  end
end;
...
procedure TPopupListboxFormEh.CMSetSizeGripChangePosition(var Message: TMessage);
begin
  if ListGrid <> nil then
    ListGrid.Perform(Message.Msg, Message.WParam, Message.LParam);
end;
