...
{ TAutoIncrementEh }
  ...
  public
    ...
    property CurValue: Integer read FCurValue;
    function Promote: Integer;
    procedure Reset;
  published
...
{$IFDEF NEXTGEN} [Weak] {$ENDIF}
  ...
  protected
    ...
    property FieldId: Largeint read FFieldId;
  public
...
{$ENDIF}
    ...
    property FieldNameUpperCase: String read FFieldNameUpperCase;
...
{ TMTDataStructEh }
  ...
  public
    ...
    function FindFieldByID(AFieldID: Largeint): TMTDataFieldEh;
    function GetCopy: TMTDataStructEh;
    function GetNextFieldID: Largeint; virtual;
    ...
    procedure EndRestructure(NewStructure: TMTDataStructEh; ApplyNewStructure: Boolean);
...
{ TMTRestructuringDataStructEh }

  TMTRestructuringDataStructEh = class(TMTDataStructEh)
  private
    FOwnerStruct: TMTDataStructEh;
  public
    constructor Create(AOwnerStruct: TMTDataStructEh); reintroduce;
    destructor Destroy; override;

    function GetNextFieldID: Largeint; override;
  end;
...
{ TOrderByList }
  ...
  TRecordsListNotifierDataEventEh =
...
{$IFDEF NEXTGEN} [Weak] {$ENDIF}
    ...
    procedure InternalMergeChanges(RemoveDeleted: Boolean);
...
{ TRecordsListNotifierEh }

  TRecordsListNotifierEh = class(TComponent)
  private
    FMemTableData: TMemTableDataEh;
    FOnAfterDataEvent: TRecordsListNotifierDataEventEh;
    FOnApplyUpdates: TRecordsListApplyUpdatesEventEh;
    FOnDataEvent: TRecordsListNotifierDataEventEh;
  ...
  protected
    ...
    procedure DataObjectChanged(); virtual;
  ...
  public
    ...
    property OnAfterDataEvent: TRecordsListNotifierDataEventEh read FOnAfterDataEvent write FOnAfterDataEvent;
    property OnApplyUpdates: TRecordsListApplyUpdatesEventEh read FOnApplyUpdates write FOnApplyUpdates;
    property OnDataEvent: TRecordsListNotifierDataEventEh read FOnDataEvent write FOnDataEvent;
...
{ TMemTableDataEh }
  ...
  private
    ...
    FNotifiers: TObjectListEh;
    FRecordsList: TRecordsListEh;
    FRestructureMode: Boolean;
  ...
  protected
    ...
    procedure AddNotifier(RecordsList: TRecordsListNotifierEh); virtual;
    ...
    procedure ReadState(Reader: TReader); override;
    procedure RecordMoved(Item: TMemoryRecordEh; OldIndex, NewIndex: Integer); virtual;
    procedure RemoveNotifier(RecordsList: TRecordsListNotifierEh);
  ...
  public
    ...
    function BeginRestruct: TMTDataStructEh; virtual; // BeginRestructure
    function FetchRecords(Count: Integer): Integer; virtual;

    procedure CancelRestruct; virtual; // CancelRestructure
    procedure CommitRestruct; virtual; // CommitRestructure
...
{$IFDEF NEXTGEN} [Weak] {$ENDIF}
    ...
    procedure SetInternalNodeExpanded(const Value: Boolean);
...
{ TMemoryTreeListEh }
  ...
  protected
    ...
    procedure SetChildVisibleForVisibleParents(Parent: TMemRecViewEh); virtual;
    procedure ChildrenVisible(Parent: TMemRecViewEh; Visible: Boolean; ARecurse: Boolean);
  ...
  public
    ...
    property FullParentCheck: Boolean read FFullParentCheck write FFullParentCheck default True;
...
{ TRecordsViewEh }
  ...
  TRecordsViewEh = class(TRecordsListNotifierEh)
  private
    ...
    FOnViewDataEvent: TRecordsListNotifierDataEventEh;
    ...
    procedure UpdateOrderCaseInsensitive;
    procedure UpdateSortedViewListIndexes;
  ...
  protected
    ...
    procedure DataObjectChanged(); override;
  ...
  public
    ...
    property OnViewDataEvent: TRecordsListNotifierDataEventEh read FOnViewDataEvent write FOnViewDataEvent;
...
function TAutoIncrementEh.Promote: Integer;
begin
  Result := FCurValue;
  Inc(FCurValue, FStep);
end;
...
begin
  ...
  FNotifiers := TObjectListEh.Create;
end;
...
begin
  ...
  while FNotifiers.Count > 0 do
    TRecordsListNotifierEh(FNotifiers[0]).DataObject := nil;
...
procedure TMemTableDataEh.RestructureData;

  function FieldId(AFieldId: Integer): TMTDataFieldEh;
  var
    i: Integer;
  begin
    Result := nil;
...
procedure TMemTableDataEh.AddNotifier(RecordsList: TRecordsListNotifierEh);
begin
  FNotifiers.Add(RecordsList);
end;

procedure TMemTableDataEh.RemoveNotifier(RecordsList: TRecordsListNotifierEh);
begin
  FNotifiers.Remove(RecordsList);
end;
...
begin
  ...
  if FNotifiers <> nil then
    for i := 0 to FNotifiers.Count-1 do
      TRecordsListNotifierEh(FNotifiers[i]).DataEvent(MemRec, Index, Action);
end;
...
begin
  Result := 0;
  for i := 0 to FNotifiers.Count-1 do
    Result := Result + TRecordsListNotifierEh(FNotifiers[i]).FetchRecords(Count);
end;
...
begin
  Result := False;
  for i := 0 to FNotifiers.Count-1 do
    if @TRecordsListNotifierEh(FNotifiers[i]).OnApplyUpdates <> nil then
    begin
      Result := True;
      Exit;
    end;
...
begin
  for i := 0 to FNotifiers.Count-1 do
    TRecordsListNotifierEh(FNotifiers[i]).ApplyUpdates(AMemTableData);
end;
...
begin
  for i := 0 to FNotifiers.Count-1 do
    TRecordsListNotifierEh(FNotifiers[i]).RecordMoved(Item, OldIndex, NewIndex);
end;
...
begin
  ...
  begin
    for i := FNotifiers.Count - 1 downto 0 do
      TRecordsListNotifierEh(FNotifiers[i]).StructChanged(Self);
  end;
end;
...
begin
  ...
  Result.FFieldId :=  OldField.FFieldId;
  FreeAndNil(OldField);
...
begin
  ...
  Field.FFieldId := GetNextFieldID;
  if FUpdateCount = 0 then
    StructChanged;
end;

function TMTDataStructEh.GetNextFieldID: Largeint;
begin
  Result := FNextFieldId;
  Inc(FNextFieldId);
end;
...
var
  I: Integer;
  AFieldNameUpperCase: string;
begin
  AFieldNameUpperCase := AnsiUpperCase(FieldName);
  for I := 0 to FList.Count - 1 do
  begin
    Result := TMTDataFieldEh(FList.Items[I]);
    if Result.FieldNameUpperCase = AFieldNameUpperCase then Exit;
  end;
  Result := nil;
end;

function TMTDataStructEh.FindFieldByID(AFieldID: Largeint): TMTDataFieldEh;
var
  I: Integer;
begin
  for I := 0 to FList.Count - 1 do
  begin
    Result := TMTDataFieldEh(FList.Items[I]);
    if Result.FFieldId = AFieldId then Exit;
  end;
  Result := nil;
end;
...
begin
  ...
  {$IFDEF FPC}
  if (Result is TMTStringDataFieldEh) then
    (Result as TMTStringDataFieldEh).Codepage := AFieldDef.Codepage;
  {$ELSE}
  {$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      BuildAndCopyDataFieldForField(Fields[i]);
    end;
  end;
end;
...
var
  ...
  DataField: TMTDataFieldEh;
begin
  Result := TMTRestructuringDataStructEh.Create(Self);
  for i := 0 to Count-1 do
  begin
    DataField := Result.CreateField(TMTDataFieldClassEh(DataFields[i].ClassType));
    DataField.FFieldId := DataFields[i].FFieldId;
...
begin
  ...
  begin
    ...
    DataFieldCorrespondenceArr[i].OldField := FindFieldByID(NewDataStruct[i].FieldId);
  end;
...
procedure TMTDataStructEh.EndRestructure(NewStructure: TMTDataStructEh;
  ApplyNewStructure: Boolean);
begin
  try
    if ApplyNewStructure then
      Assign(NewStructure);
  finally
    FreeAndNil(NewStructure);
  end;
end;
...
{$ENDREGION 'TMTDataStructEh'}

{$REGION 'TMTRestructuringDataStructEh'}

{ TMTRestructuringDataStructEh }

constructor TMTRestructuringDataStructEh.Create(AOwnerStruct: TMTDataStructEh);
begin
  inherited Create(nil);
  FOwnerStruct := AOwnerStruct;
end;

destructor TMTRestructuringDataStructEh.Destroy;
begin
  inherited Destroy;
end;

function TMTRestructuringDataStructEh.GetNextFieldID: Largeint;
begin
  Result := FOwnerStruct.GetNextFieldID;
end;

{$ENDREGION 'TMTRestructuringDataStructEh'}
...
var
  ...
  function IsUnique(const ACompName: string): Boolean;
  var
  ...
  begin
    ...
    if OwnerDataStruct.FindComponent(ACompName) = nil
      then Result := True
      else Result := False;
...
begin
  ...
  begin
    FFieldName := Value;
    FFieldNameUpperCase := AnsiUpperCase(FFieldName);
  end else
  begin
    CheckInactive;
...
{$ENDIF}
{$IFDEF EH_LIB_37}
    ftLargeUint: FNumericDataType := fdtLargeUintEh;
{$ENDIF}
  else
    raise Exception.Create('Can not assign DataType from Field');
  end;
...
begin
  InternalMergeChanges(True);
end;

procedure TMemoryRecordEh.InternalMergeChanges(RemoveDeleted: Boolean);
begin
...
constructor TRecordsListNotifierEh.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
end;

destructor TRecordsListNotifierEh.Destroy;
begin
  DataObject := nil;
  inherited Destroy;
end;

procedure TRecordsListNotifierEh.DataEvent(MemRec: TMemoryRecordEh;
...
procedure TRecordsListNotifierEh.AfterDataEvent(MemRec: TMemoryRecordEh;
  Index: Integer; Action: TRecordsListNotification);
begin
...
procedure TRecordsListNotifierEh.RecordAdded(MemRec: TMemoryRecordEh; Index: Integer);
begin
end;

procedure TRecordsListNotifierEh.RecordChanged(MemRec: TMemoryRecordEh; Index: Integer);
begin
end;

procedure TRecordsListNotifierEh.RecordDeleted(MemRec: TMemoryRecordEh; Index: Integer);
begin
end;

procedure TRecordsListNotifierEh.RecordListChanged;
begin
end;

{procedure TRecordsListNotifierEh.SetRecordsList(const Value: TRecordsListEh);
begin
  if Value = FRecordsList then Exit;
  if FRecordsList <> nil then FRecordsList.RemoveNotifier(Self);
  if Value <> nil then Value.AddNotifier(Self);
  FRecordsList := Value;
end;}

procedure TRecordsListNotifierEh.SetMemTableData(const Value: TMemTableDataEh);
begin
  if Value = FMemTableData then Exit;
  DataEvent(nil, -1, rlnListChangingEh);
  if FMemTableData <> nil then
  begin
    FMemTableData.RemoveNotifier(Self);
    FMemTableData.RemoveFreeNotification(Self);
  end;
  if Value <> nil then
  begin
    Value.AddNotifier(Self);
...
procedure TRecordsListNotifierEh.SetDataObject(const Value: TComponent);
begin
...
procedure TRecordsListNotifierEh.DataObjectChanged;
begin
end;

procedure TRecordsListNotifierEh.Notification(AComponent: TComponent;
...
function TRecordsListNotifierEh.FetchRecords(Count: Integer): Integer;
begin
  Result := 0;
...
procedure TRecordsListNotifierEh.ApplyUpdates(
...
procedure TRecordsListNotifierEh.RecordMoved(Item: TMemoryRecordEh;
...
procedure TRecordsListNotifierEh.StructChanged(AMemTableData: TMemTableDataEh);
begin
  if Assigned(OnStructChanged) then
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (Rec <> nil) then
          Rec.InternalMergeChanges(False);
      end;
    end;
    FreeDeletedRecords;
    PackRecList;
...
begin
  ...
  begin
    ...
    FDataType := TAggregateField(Source).ResultType;
  end
  {$ENDIF}
  else
    inherited Assign(Source);
...
begin
  ...
  for i := 0 to FNotifiers.Count-1 do
    TRecordsListNotifierEh(FNotifiers[i]).RecordMoved(MemRec, OldIndex, NewIndex);
end;
  ...
  case Action of
    ...
    rlnRecChangedEh, rlnRecMarkedForDelEh:
      begin
        ...
        begin
          IdxInSortedList := SearchRec(FSorderViewList, MemRec);
          if IdxInSortedList >= 0 then
            RecView := SortedListItem[IdxInSortedList]
          else
            RecView := nil;
        end;
...
begin
  ...
  begin
    ...
    PrefilteredList := TObjectListEh.Create;
    try
      ...
      begin
        for i := 0 to PrefilteredList.Count-1 do
          AddRecInMemoryViewList(TMemRecViewEh(PrefilteredList[i]), FMemoryViewList.Count);
      end;
    finally
      PrefilteredList.Free;
    end;
...
begin
  ...
  if not MemoryTreeList.FilterNodeIfParentVisible then
    MemoryTreeList.SetChildVisibleForVisibleParents(nil);
  if FSortOrder <> '' then
    ResortTreeList;
end;
...
begin
  ...
  UpdateSortedViewListIndexes;

  Notify(nil, -1, rlnListChangedEh);
end;

procedure TRecordsViewEh.UpdateSortedViewListIndexes;
var
  i: Integer;
begin
  for i := 0 to FSorderViewList.Count-1 do
...
begin
  ...
  else raise Exception.Create('Unsupported aggregate function: ' + AggrFuncName);
                ...
                begin
                  ...
                  case FuncType of
                    eftMin:
                      ...
                      else if VarIsNull(Result_Right) and
                              not VarIsNull(ListBuffer[0]) then
                      begin
                        Result_Right := ListBuffer[0];
                      end
                      else if not VarIsNull(ListBuffer[0]) and
                              (ListBuffer[0] < Result_Right) then
                      begin
                        Result_Right := ListBuffer[0];
                      end;
...
begin
  if MemTableData = nil then
  begin
    SetLength(FTreeViewKeyFields, 0);
    SetLength(FTreeViewRefParentFields, 0);
  end else
  begin
...
procedure TRecordsViewEh.DataObjectChanged;
begin
  inherited DataObjectChanged;
  UpdateFields;
end;
...
procedure TRecordsViewEh.AddNotifier(RecordsList: TRecordsListNotifierEh);
begin
  FNotifiers.Add(RecordsList);
end;

procedure TRecordsViewEh.RemoveNotifier(RecordsList: TRecordsListNotifierEh);
begin
  FNotifiers.Remove(RecordsList);
end;
...
var
  ...
  MemTlb: TCustomMemTableEhCrack;
begin
  MemTlb := TCustomMemTableEhCrack(FDataSet);
  SaveState := MemTlb.SetTempState(dsFilter);

  MemTlb.RecordToBuffer(Rec, dvvValueEh, {$IFDEF NEXTGEN}TRecBuf{$ELSE}TRecordBuffer{$ENDIF}(MemTlb.TempBuffer), -1);
  MemTlb.BufferToRecBuf({$IFDEF NEXTGEN}TRecBuf{$ELSE}TRecordBuffer{$ENDIF}(MemTlb.TempBuffer)).NeedUpdateCalcFields := True;

  Result := IsCurRecordBufInFilter(TCustomMemTableEhCrack(FDataSet).TempBuffer);
  TCustomMemTableEhCrack(FDataSet).RestoreState(SaveState);
end;
{$ELSE}
begin
...
begin
  ...
  Result.SetInternalNodeExpanded(DefaultNodeExpanded);
...
begin
  ...
  begin
    ...
    if (ParentNode = nil) and (FInsertedNodeStack.Count > 0) then
    begin
  ...
  begin
    if FullParentCheck  then
    begin

      if MemRecord.RecordsList.Indexes.Count > 0
        then MTIndex := MemRecord.RecordsList.Indexes.GetIndexForFields(KeyFieldNames)
        else MTIndex := nil;
      if (MTIndex <> nil) and MTIndex.Active and (MTIndex.Fields <> '') then
      begin
        ParentNode := nil;
...
procedure TMemoryTreeListEh.SetChildVisibleForVisibleParents(Parent: TMemRecViewEh);
var
  i: Integer;
begin
  if Parent = nil then
  ...
  begin
    if Parent[i].Visible and (Parent[i].VisibleCount = 0)  then
      ChildrenVisible(Parent[i], True, True)
    else if Parent[i].Visible then
      SetChildVisibleForVisibleParents(Parent[i]);
  end;
end;

procedure TMemoryTreeListEh.ChildrenVisible(Parent: TMemRecViewEh; Visible: Boolean; ARecurse: Boolean);
var
  i: Integer;
begin
  if Parent = nil then
  ...
  begin
    ...
    if ARecurse then
      ChildrenVisible(Parent[i], Visible, ARecurse);
  end;
end;
...
procedure TMemoryTreeListEh.TreeChanged(Node: TBaseTreeNodeEh;
...
begin
  ...
  if Operation in [tlnNodeDeletedEh, tlnTreeListChangedEh] then
    FInsertedNodeStack.Clear;
end;
...
begin
  Result := TMemoryTreeListEh(inherited TreeList);
end;
...
procedure TMemRecViewEh.SetInternalNodeExpanded(const Value: Boolean);
begin
  FLockUpdate := True;
  try
    Expanded := Value;
